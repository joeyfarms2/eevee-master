function showSearchResult(){
	...
	if (jQuery("#frm_report").valid())
	{
		...
		var full_url = base_url+"admin/report/news-log/ajax-get-news-log-list/"+sid+"/owner";
		// alert(full_url);
...
function exportToExcel(){
	...
	if (jQuery("#frm_report").valid())
	{
		...
		var sid = Math.floor(Math.random()*10000000000);	  
		var full_url = base_url+"admin/report/news-log/export-news-log?created_date_from="+created_date_from+"&created_date_to="+created_date_to+"&search_order_by="+search_order_by;
		// alert(full_url);
