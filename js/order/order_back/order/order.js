function showSearchResult(){
	...
	var full_url = base_url+"admin/order/ajax-get-main-list/"+sid;
	// alert(full_url);
...
function clearSearchResult(){
	...
	$("#search_need_transport").attr("checked", false);

	$("#search_type_all").attr('checked', true);
	$("input[name='search_type[]']").each(function()
	{
		this.checked = true;	
	});
...
function checkToggleAdvanceSearch(){
	...
	var search_status_all = true;
	$("input[name='search_status[]']").each(function()
	{
		if(this.checked){
			search_status_all = false;
		}
	});

	var search_need_transport = $("#search_need_transport:checked").val();
	var search_type_all = $("#search_type_all:checked").val();

	if( search_post_word == "" && search_post_word == "" && created_date_from == "" && created_date_to == "" && search_status_all && search_need_transport != '1' && search_type_all){
		set_toggle_advance_search('hide');
	}else{
