function showSearchResult(){
	...
	var full_url = base_url+"admin/questionaire-category/ajax-get-main-list/"+sid;
	// alert(full_url);
	...
	search_questionaire_main = new Array();
	$("input[name='search_questionaire_main[]']").each(function()
	{
		if(this.checked){
			search_questionaire_main.push(this.value);
		}	
	});
	
	$.getJSON(full_url, ({ search_record_per_page:search_record_per_page,search_order_by:search_order_by,page_selected:page_selected,search_post_word:search_post_word, created_date_from:created_date_from, created_date_to:created_date_to, search_in:search_in, search_status:search_status, search_questionaire_main:search_questionaire_main }) , 
		function(data){
			if (null != data) {
