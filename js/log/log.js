function showSearchResult(){
	...
	var full_url = base_url+"admin/log/ajax-get-main-list/"+sid;
	// alert(full_url);
	...
	jQuery("input[name='search_role[]']").each(function()
	{
		if(this.checked){
	...
	$.getJSON(full_url, ({ search_record_per_page:search_record_per_page,search_order_by:search_order_by,page_selected:page_selected,search_post_word:search_post_word, created_date_from:created_date_from, created_date_to:created_date_to, search_in:search_in, search_role:search_role }) , 
		function(data){
			if (null != data) {
...
function checkToggleAdvanceSearch(){
	...
	if( search_post_word == "" && search_post_word == "" && created_date_from == "" && created_date_to == "" && search_role_all){
		set_toggle_advance_search('hide');
	}else{
