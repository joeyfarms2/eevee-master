...
function draw_transaction_result(data,divName,show_i){
	if(data.status=="error"){
		...
		txt += '<thead>';
		txt += '<tr role="row">';
		txt += '<th class="hcenter">Barcode</th>';
		txt += '<th class="hcenter">Title</th>';
		...
		txt += '<div><i>*The transactions above are not saved until you click on “Confirm” button for confirmation.</i></div>';
...
function sendOverdueEmail(transaction_aid){
	var sid = Math.floor(Math.random()*10000000000);	  
	var full_url = base_url+"admin/transactions/ajax-send-mail-overdue/"+sid;

	$.ajax({
		type: "POST",
		url: full_url,
		data: ({aid_selected:transaction_aid }),
		dataType: "json",
		beforeSend: function(data) {
			$('#search_product_button').removeClass('fa-search').addClass('fa-spinner');
		},
