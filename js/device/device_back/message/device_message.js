function showSearchResult(){
	...
	var full_url = base_url+"admin/device-message/ajax-get-main-list/"+sid;
	// alert(full_url);
...
function processSendMessage(aid, type){
	var sid = Math.floor(Math.random()*10000000000);	  
	var full_url = base_url+"admin/device-message/ajax-send-message/"+sid;
	$.ajax({
		type: "POST",
		url: full_url,
		data: ({device_message_aid:aid, device_message_type:type }),
		dataType: "json",
...
function processSendMessageWithTestPem(aid, type){
	var sid = Math.floor(Math.random()*10000000000);	  
	var full_url = base_url+"admin/device-message/ajax-send-message-with-test-pem/"+sid;
	$.ajax({
		type: "POST",
		url: full_url,
		data: ({device_message_aid:aid, device_message_type:type }),
