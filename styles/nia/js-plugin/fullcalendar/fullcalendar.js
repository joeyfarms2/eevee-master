...
function Header(calendar, options) {
	...
	function updateTitle(text) {
		...
		var tmp2 = text.split(",");
		if (tmp.length == 2) {
			el.find('h2').html("<span class='textStart'>"+tmp[0]+"</span> <span class='textSub'>"+tmp[1]+"</span>");
		}
		else if (tmp2.length == 2) {
			el.find('h2').html("<span class='textStart'>"+tmp2[0]+",</span> <span class='textSub'>"+tmp2[1]+"</span>");
		}
		else {
