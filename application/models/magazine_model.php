...
class Magazine_model extends Product_init_model {
	
	function __construct()
	{
		// Call the Model constructor
        parent::__construct();
		$this->set_table_name("magazine");
	...
	function set_join_for_desc($obj=""){
		$this->db->select('magazine.*, publisher.name as publisher_name, publisher.status as publisher_status, product_main.name as product_main_name, product_main.url as product_main_url, product_main.icon as icon, product_type.cid as product_type_cid');
		$this->db->join($this->tbl_publisher_name.' AS publisher', 'magazine.publisher_aid = publisher.aid', "left");
		$this->db->join($this->tbl_product_main_name.' AS product_main', 'magazine.product_main_aid = product_main.aid', "left");
		$this->db->join($this->tbl_product_type_name.' AS product_type', 'magazine.product_type_aid = product_type.aid', "left");
	}
	...
	function update_parent($aid){
		...
		$_sql ="
			UPDATE ".$this->get_table_name()." SET total_copy = ( 
				SELECT count( aid ) 
				FROM ".$this->tbl_magazine_copy_name." 
		...
		$CI =& get_instance();
		$CI->load->model($this->magazine_copy_model,"magazine_copy");
		$tmp = array();
		//$tmp["product_type_cid"] = 'magazine';
		$tmp["parent_aid"] = $aid;
		$tmp["status"] = "1";
		$CI->magazine_copy->set_where($tmp);
		$copy_result = $CI->magazine_copy->load_records(true);
		$has_ebook = 0;
		$has_license = 0;
		$best_price = 0;
...
/* Location: ./system/application/model/magazine_model.php */