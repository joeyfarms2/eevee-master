...
class Product_init_model extends Initmodel {
				...
				foreach ($copy_result_show as $key => $value) {
					...
					switch ($type_minor) {
						...
						case '5':
							...
							$row["copy_type_minor_show"] .= '<i class="fa fa-book"></i> Knowledge Resources';
							break;
						case '6':
							if(!is_blank($row["copy_type_minor_show"])){
								$row["copy_type_minor_show"] .= '&nbsp;/&nbsp;';
						...
						case '8':
							...
							$row["copy_type_minor_show"] .= '<i class="fa fa-book"></i> Information';
							break;
						case '9':
							if(!is_blank($row["copy_type_minor_show"])){
								$row["copy_type_minor_show"] .= '&nbsp;/&nbsp;';
							}
							$row["copy_type_minor_show"] .= '<i class="fa fa-video-camera"></i> Multimedia';
							break;
					}
					if($value > 1){
	...
	function manage_copy_result($row){
		...
		$shelf_name = get_array_value($row,"shelf_name","");
		if(strlen($shelf_name) != 1){
			$row["shelf_location_name"] = "( ".$shelf_name." )";
		}else{
			switch($shelf_name){
				case "1" : $row["shelf_location_name"] = "( ห้องสมุดศูนย์ความรู้จังหวัดชายแดนภาคใต้ )"; break;
				case "2" : $row["shelf_location_name"] = "( ห้องสมุดศูนย์ฝึกอบรมประชานาถ )"; break;
				case "3" : $row["shelf_location_name"] = "( ห้องสมุดสำนักข่าวกรองแห่งชาติ )"; break;
				default : $row["shelf_location_name"] = "N/A"; break;
			}
		}
	...
	function manage_field_result($row){
		...
		switch ($tag) {
			...
			case '050':
				$field_data_link = "";
				if (get_array_value($row,"subfield_cd","") == "a") {
					$field_data_link .= '<a href="'.site_url('search/'.$field_data.'?search_in=call_number').'">'.$field_data.'</a>';
				}else{
					$field_data_link .= $field_data;
				}
			...
			case '700':
				...
				if(is_var_array($data)){
					foreach($data as $item){
						$item = trim($item);
						$item_link = preg_replace('/[^a-zA-Z0-9ก-ฮ๐-๙\-.ๅุูึๆไใำะัํี๊ฯโเ้็่๋าแิฺื์\s]/i', '', $item);
						if(!is_blank($item)){
							if(!is_blank($field_data_link)){
								$field_data_link .= '  ';
							}
							$field_data_link .= '<a href="'.site_url('search/'.$item_link.'/option-and?search_in=subject').'">'.$item.'</a>';
						}
					}
				}else{
					$item_link = preg_replace('/[^a-zA-Z0-9ก-ฮ๐-๙\-.ๅุูึๆไใำะัํี๊ฯโเ้็่๋าแิฺื์\s]/i', '', $field_data);
					$field_data_link = '<a href="'.site_url('search/'.$item_link.'/option-and?search_in=subject').'">'.$field_data.'</a>';
				}
				$row["field_data_link"] = $field_data_link;
				break;
