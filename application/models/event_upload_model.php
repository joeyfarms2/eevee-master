<?php
class Event_upload_model extends Initmodel {
	
	function __construct()
	{
		// Call the Model constructor
        parent::__construct();
		$this->set_table_name("event_upload");	
	...
	function set_join_for_desc($obj=""){
		$this->db->select($this->get_table_name().'.*, event_main.name as event_main_name');
		$this->db->join($this->tbl_event_main_name.' AS event_main', $this->get_table_name().'.event_main_aid = event_main.aid', "left");
	}
	...
	function fetch_data_with_desc($query)
	{
		...
		if($query->num_rows() > 0){
			$this->db->flush_cache();
			$CI2 =& get_instance();
			$CI2->load->model($this->event_category_model,"category");
			...
			foreach($query->result_array() as $row){
				...
				$event_title = get_array_value($row,"title","");
				$row["shot_title"] = getShortString($event_title,"35");
				$row["shot_title_focus"] = getShortString($event_title,"70");
...
/* End of file event_upload_model.php */
/* Location: ./system/application/model/event_upload_model.php */