...
class Others_model extends Product_init_model {
	
	function __construct()
	{
		// Call the Model constructor
        parent::__construct();
		$this->set_table_name("others");
		...
		$this->tbl_others_copy_name = "others_copy";
		$this->tbl_product_category_name = "product_category";
		
	}
	
	function set_join_for_desc($obj=""){
		$this->db->select('others.*, publisher.name as publisher_name, publisher.status as publisher_status, product_main.name as product_main_name, product_main.url as product_main_url, product_main.icon as icon, product_type.cid as product_type_cid');
		$this->db->join($this->tbl_publisher_name.' AS publisher', 'others.publisher_aid = publisher.aid', "left");
		$this->db->join($this->tbl_product_main_name.' AS product_main', 'others.product_main_aid = product_main.aid', "left");
		$this->db->join($this->tbl_product_type_name.' AS product_type', 'others.product_type_aid = product_type.aid', "left");
	}
	...
	function update_parent($aid){
		...
		$_sql ="
			UPDATE ".$this->get_table_name()." SET total_copy = ( 
				SELECT count( aid ) 
				FROM ".$this->tbl_others_copy_name." 
		...
		$CI =& get_instance();
		$CI->load->model($this->others_copy_model,"others_copy");
		$tmp = array();
		$tmp["product_type_aid"] = '4';
		$tmp["parent_aid"] = $aid;
		$tmp["status"] = "1";
		$CI->others_copy->set_where($tmp);
		$copy_result = $CI->others_copy->load_records(true);
		$has_ebook = 0;
		$has_license = 0;
		$best_price = 0;
...
/* End of file others_model.php */
/* Location: ./system/application/model/others_model.php */