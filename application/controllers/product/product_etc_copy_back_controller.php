...
class Product_etc_copy_back_controller extends Product_init_controller {
	...
	function __construct(){
		...
		$this->main_model = 'Etc_copy_model';
		
		$this->lang->load('product');
		$this->lang->load('product_etc');		
	}
	
	function check_exits_etc($parent_aid="",$return_json=false){
		if(!is_number_no_zero($parent_aid)){
			redirect('admin/product-'.$product_main_url.'/etc/status/'.md5('no-etc'));
			return"";
		}
		$this->load->model($this->etc_model,'etc');
		$this->etc->set_where(array("aid"=>$parent_aid, "user_owner_aid"=>getUserOwnerAid($this)));
		$parent_detail = $this->etc->load_record(true);
	...
	function show($product_main_url="", $parent_aid="", $msg=""){
		...
		$this->data["view_the_content"] = $this->default_theme_admin . '/' . folderName . '/etc_copy_list';
		...
		$parent_detail = $this->check_exits_etc($parent_aid, false);
	...
	function form($product_main_url="", $parent_aid=""){
		define("thisAdminTabMenu",'product_etc_'.$product_main_url);
		$this->data["title"] = DEFAULT_TITLE;
		$this->data["view_the_content"] = $this->default_theme_admin . '/' . folderName . '/etc_copy_form';
	...
	function edit($product_main_url="", $parent_aid="", $aid=""){
		...
		if(is_var_array($copy_item_detail)){
			...
			$this->data["message"] = set_message_error('Cannot find the specific etc copy.');
			$this->data["js_code"] = "";
	...
	function save(){
		...
		if($command == "_update"){
			...
			if(!is_var_array($itemResult)){
				redirect('admin/product-'.$product_main_url.'/etc/edit/'.$parent_aid.'/copy/status/'.md5('no-etc-copy'));
				return "";
			}
		...
		if(!is_in_array($type, $type_master)){
			redirect('admin/product-'.$product_main_url.'/etc/status/'.md5('copy-type-incorrect'));
			return "";
		}
		if(!is_in_array($possession, $possession_master)){
			redirect('admin/product-'.$product_main_url.'/etc/status/'.md5('copy-possession-incorrect'));
			return "";
		}
		...
		if($type == '1'){ // Digital
			...
			if($is_license == '1'){
				if($possession == '1'){
					redirect('admin/product-'.$product_main_url.'/etc/status/'.md5('both-buyout-license'));
					return "";
				}
			...
			$digital_point = "";
			if($is_license == '1'){
				redirect('admin/product-'.$product_main_url.'/etc/status/'.md5('both-paper-license'));
				return "";
			}else{
				$is_license = "";
				$ebook_concurrence = "";
		...
		if(is_blank($barcode)){
			...
			}while( $this->isEtcBarcodeExits($barcode, "") );
			$data["barcode"] = $barcode;
			// $data["barcode_no"] = $value;
		}else{
			$data["barcode"] = $barcode;
			if($this->isEtcBarcodeExits($barcode, $aid)){
		...
		if(is_blank($upload_path)){
			$upload_path = "uploads/".CONST_PROJECT_CODE."/etc_copy/".get_datetime_pattern("Y",$today_date,"").'/'.get_datetime_pattern("m",$today_date,"").'/'.$barcode.'_'.$cid.'/';
		}
		...
		if( !is_blank(get_array_value($_FILES,"file_upload","")) && !is_blank(get_array_value($_FILES["file_upload"],"name","")) ){
			...
			if ( !is_blank(get_array_value($result_file_upload,"error_msg","")) )
			{
				...
				$this->log_error('Admin : Product copy', 'Save etc copy fail => Upload file error : '.$result_file_upload["error_msg"]);
		...
		if($command == "_insert"){
			...
			if($aid){
				$this->load->model($this->etc_model,'etc');
				$this->etc->update_parent($parent_aid);
				$this->log_status('Backend : Insert etc copy', '[barcode='.$barcode.'] of ['.$title.'] just added into database.', $data);
				$this->log_product_save($product_type_aid, $parent_aid, $title, "U", "Etc copy [barcode = $barcode] has been added", $data, "copy-add");
				redirect('admin/product-'.$product_main_url.'/etc/edit/'.$parent_aid.'/copy/status/'.md5('success'));
			}else{
				$this->log_error('Backend : Insert etc copy', 'Command insert_record() fail. Can not insert [barcode='.$barcode.'] of ['.$title.']', $data);
			...
			if($rs){
				...
				$this->log_status('Admin : Update etc copy',  '[barcode='.$barcode.'] of ['.$title.'] has been updated.', $data);
				$this->log_product_save($product_type_aid, $parent_aid, $title, "U", "Etc copy [barcode = $barcode] has been updated", $data, "copy-update");
				if($save_option){
					redirect('admin/product-'.$product_main_url.'/etc/edit/'.$parent_aid.'/copy/add');
				}else{
					redirect('admin/product-'.$product_main_url.'/etc/edit/'.$parent_aid.'/copy/status/'.md5('success'));
				}
				return "";
			}else{
				$this->log_error('Backend : Update etc copy', 'Command update_record() fail. Can not update [barcode='.$barcode.'] of ['.$title.']', $data);
				$this->data["message"] = set_message_error('Sorry, The system can not insert data right now.<BR>Please try again later or contact administrator to solve the problem.');
				$this->data["js_code"] = '';
				$this->data["command"] = $command;
				$this->data["copy_item_detail"] = $data;
			...
			$this->log_error('Backend : Etc copy', 'Command not found.', $command);
			redirect('admin/product-'.$product_main_url.'/etc/edit/'.$parent_aid.'copy/status/'.md5('no-command'));
			return "";
		}
	}
	...
	function ajax_set_value($sid=""){
		...
		if ($rs){
			$this->log_status('Backend : Etc copy', 'Set value : "'.$f_name.'" has been set to "'.$f_value.'" for '.$this_obj_info.'.', $objResult);
			$msg = set_message_success('Data has been saved.');
			$result_obj = array("status" => 'success',"msg" => $msg );
			echo json_encode($result_obj);
			return "";
		}else{
			$this->log_error('Backend : Etc copy', 'Set value : Function main->set_value() fail. Can not set "'.$f_name.'" to "'.$f_value.'" for '.$this_obj_info.'.', $objResult);
	...
	function ajax_delete_one(){
		...
		$objResult = $this->main->load_record(false);
		if(!is_var_array($objResult)){
			$msg = set_message_error('Error occurred. Can not find this etc copy.');
			$result_obj = array("status" => 'error',"msg" => $msg );
			echo json_encode($result_obj);
			return "";
		}
		...
		if ($rs){
			...
			$this->log_status('Backend : Delete etc copy', $this_obj_info.' has been deleted.', $objResult);
			$this->log_product_save($product_type_aid, $parent_aid, $title, "U", "Etc copy [barcode = $this_name] has been deleted", $objResult, "copy-delete");
	...
	function manage_column_detail($result_list, $product_main_url="0"){
		...
		foreach($result_list as $item){
			...
			$item["name_action"] = '<a href="'.site_url('admin/product-'.$product_main_url.'/etc/edit/'.$parent_aid.'/copy/edit/'.get_array_value($item,"aid","")).'">'.get_array_value($item,"barcode","-").'</a>';
			
			$status = get_array_value($item,"status","0");
			if($status == 1){
				$item["status_action"] = '<a class="btn btn-success btn-xs" title="Click to \'Inctive\' this etc." onclick="processChangeValue(\' inactive <strong>'.removeAllQuote(get_array_value($item,"barcode","")).'</strong>\', \'admin/product/etc-copy\', \''.get_array_value($item,"aid","").'\', \'status=0\')"><i class="fa fa-check"></i></a>';
			}else{
				$item["status_action"] = '<a class="btn btn-danger btn-xs" title="Click to \'Active\' this etc." onclick="processChangeValue(\' active <strong>'.removeAllQuote(get_array_value($item,"barcode","")).'</strong>\', \'admin/product/etc-copy\', \''.get_array_value($item,"aid","").'\', \'status=1\')"><i class="fa fa-ban"></i></a>';
			}
			...
			$item["action"] = '';
			$product_type_field_aid = get_array_value($item,"product_type_field_aid","0");
			if($product_type_field_aid <= 0 && (CONST_ALLOW_DELETE_PRODUCT != "0" || is_root_admin_or_higher())){
				$item["action"] .= '<a class="btn btn-danger btn-xs" title="Click to \'Delete\' this etc." onclick="processDelete(\''.get_array_value($item,"aid","").'\', \'admin/product/etc-copy\', \'<strong>'.removeAllQuote(get_array_value($item,"barcode","")).'</strong>\')"><i class="fa fa-trash-o "></i></a>&nbsp;&nbsp;&nbsp;';
			}
	...
	function status($product_main_url="", $parent_aid="", $type="")	{
		switch($type)
		{
			...
			case md5('no-command') : 
				...
				$this->data["js_code"] = '';
				break;
			case md5('no-etc-copy') : 
				$this->data["message"] = set_message_error('Error occurred. Can not find this etc copy.');
				$this->data["js_code"] = '';
				break;
			default : 
	...
	function isEtcBarcodeExits($code, $aid){
		$this->load->model($this->main_model,"main");
