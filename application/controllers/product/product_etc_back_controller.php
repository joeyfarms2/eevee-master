...
class Product_etc_back_controller extends Product_init_controller {
	...
	function __construct(){
		...
		$this->lang->load('product_etc');

		$this_product_type_aid = '4';
		$this->data["this_product_type_aid"] = $this_product_type_aid;

		define("thisAdminSubMenu",'general_info');
		@define("folderName",'product/product_back/etc');
		
		define("TXT_TITLE",'Etc management');
		define("TXT_INSERT_TITLE",'Etc form : Add new etc');
		define("TXT_UPDATE_TITLE",'Etc form : Edit etc');
				
		$this->main_model = 'Etc_model';
	}
	...
	function show($product_main_url=""){
		define("thisAdminTabMenu",'product_etc_'.$product_main_url);
		@define("thisAction",'show');
		$this->data["title"] = DEFAULT_TITLE;
		$this->data["view_the_content"] = $this->default_theme_admin . '/' . folderName . '/etc_list';
	...
	function form($product_main_url=""){
		define("thisAdminTabMenu",'product_etc_'.$product_main_url);
		$this->data["title"] = DEFAULT_TITLE;
		$this->data["view_the_content"] = $this->default_theme_admin . '/' . folderName . '/etc_form';
	...
	function add($product_main_url=""){
		...
		$this->session->set_userdata('productEtcBackDataSearchSession','');
	...
	function edit($product_main_url="", $aid=""){
		...
		if(is_var_array($parent_detail)){
			...
			$this->data["message"] = set_message_error('Cannot find the specific etc.');
			$this->data["js_code"] = '';
			$this->show($product_main_url);
			return "";
		}
	...
	function save(){
		...
		if($command == "_insert"){
			...
			if($aid > 0){
				...
				$etc_obj = $this->save_field($product_type_aid,$aid);
				$name = get_array_value($etc_obj,"title","-");
				$author = get_array_value($etc_obj,"author","");
				$data["aid"] = $aid;
				$this->log_status('Backend : Insert etc', '['.$name.']['.$aid.'] just added into database.', $data);
				$this->log_product_save($product_type_aid, $aid, $name, "I", "New etc has been added.", $data, "etc-add");
				$path = "uploads/".CONST_PROJECT_CODE."/etc/".ceil($aid/100);
				$data = $this->upload_cover_image($aid, $data, $path);
				if ( !is_blank(get_array_value($data,"error_msg","")) )
				{
					// echo $data["error_msg"];
					$this->log_status('Backend : Insert etc', 'Save etc fail => Upload image error : '.$data["error_msg"], $data);
				...
				redirect('admin/product-'.$product_main_url.'/etc/edit/'.$aid.'/copy/add');
			}else{
				$this->log_error('Backend : Insert etc', 'Command insert_record() fail. Can not insert '.$name, $data);
			...
			if($rs){
				...
				if ($aid > 0) $this->log_status('Admin : Update etc',  '['.$name.']['.$aid.'] has been updated.', $data);
				$this->log_product_save($product_type_aid, $aid, $name, "U", "General info has been updated.", $data, "etc-update");
				$path = "uploads/".CONST_PROJECT_CODE."/etc/".ceil($aid/100);
				$data = $this->upload_cover_image($aid, $data, $path);
				if ( !is_blank(get_array_value($data,"error_msg","")) )
				{
					// echo $data["error_msg"];
					$this->log_status('Backend : Update etc', 'Save etc fail => Upload image error : '.$data["error_msg"], $data);
					$this->data["message"] = set_message_error(get_array_value($data,"error_msg","Sorry, the system can not save data now. Please try again or contact your administrator."));
					$this->data["js_code"] = "";
					$this->data["command"] = $command;
					$this->data["parent_detail"] = $data;
				...
				if($save_option){
					redirect('admin/product-'.$product_main_url.'/etc/add');
				}else{
					redirect('admin/product-'.$product_main_url.'/etc/edit/'.$aid);
					// redirect('admin/product-'.$product_main_url.'/etc/status/'.md5('success'));
				}
				return "";
			}else{
				$this->log_error('Backend : Update etc', 'Command update_record() fail. Can not update '.$name.'['.$aid.']', $data);
				$this->data["message"] = set_message_error('Sorry, the system can not save data now. Please try again or contact your administrator.');
				$this->data["js_code"] = '';
				$this->data["command"] = $command;
				$this->data["parent_detail"] = $data;
			...
			$this->log_error('Backend : Etc', 'Command not found.', $data);
			redirect('admin/product-'.$product_main_url.'/etc/status/'.md5('no-command'));
			return "";
		}
	}
	...
	function ajax_set_value($sid=""){
		...
		if(!is_var_array($objResult)){
			$msg = set_message_error('Error occurred. Can not find this etc.');
			$result_obj = array("status" => 'error',"msg" => $msg );
			echo json_encode($result_obj);
			return "";
		}
		...
		if ($rs){
			$this->log_status('Backend : Etc', 'Set value : "'.$f_name.'" has been set to "'.$f_value.'" for '.$this_obj_info.'.', $objResult);
			$msg = set_message_success('Data has been saved.');
			$result_obj = array("status" => 'success',"msg" => $msg );
			echo json_encode($result_obj);
			return "";
		}else{
			$this->log_error('Backend : Etc', 'Set value : Function main->set_value() fail. Can not set "'.$f_name.'" to "'.$f_value.'" for '.$this_obj_info.'.', $objResult);
	...
	function ajax_delete_one(){
		...
		$this->load->model($this->etc_copy_model,'copy');
		$this->copy->set_where(array("parent_aid"=>$aid, "user_owner_aid"=>$user_owner_aid));
		$etc_copy_list = $this->copy->load_records(false);
		if(is_var_array($etc_copy_list)){
			foreach($etc_copy_list as $item){
		...
		if ($rs){
			$this->log_status('Backend : Delete etc', $this_obj_info.' has been deleted.', $objResult);
			$this->log_product_save($this->data["this_product_type_aid"], $aid, $this_obj_title, "D", "Etc has been deleted.", $objResult, "etc-delete");
			$msg = set_message_success($this_obj_title.' has been deleted.');
			$result_obj = array("status" => 'success',"msg" => $msg );
			echo json_encode($result_obj);
			return "";
		}else{
			$this->log_error('Backend : Delete etc', 'Command delete_records() fail. Can not delete '.$this_obj_info.'.', $objResult);
			$msg = set_message_error('Sorry, the system can not save data now. Please try again or contact your administrator.');
			$result_obj = array("status" => 'error',"msg" => $msg );
			echo json_encode($result_obj);
			return "";
	...
	function ajax_get_main_list($sid){
		...
		$dsSession = $dataSearchSession->userdata('productEtcBackDataSearchSession');		
		// print_r($dsSession);
		...
		$header_list[] = array("sort_able"=>'1',"title_show"=>'Etc Name',"field_order"=>'title',"col_show"=>'title_action',"title_class"=>'w150 a-center',"result_class"=>'a-left');
		...
		$header_list[] = array("sort_able"=>'1',"title_show"=>'New',"field_order"=>'is_new',"col_show"=>'is_new_action',"title_class"=>'hidden-xs w50 a-center',"result_class"=>'hidden-xs a-left');
		$header_list[] = array("sort_able"=>'1',"title_show"=>'Rec.',"field_order"=>'is_recommended',"col_show"=>'is_recommended_action',"title_class"=>'hidden-xs w50 a-center',"result_class"=>'hidden-xs a-left');
		$header_list[] = array("sort_able"=>'1',"title_show"=>'In Home',"field_order"=>'is_home',"col_show"=>'is_home_action',"title_class"=>'hidden-xs w50 a-center',"result_class"=>'hidden-xs a-left');
		$header_list[] = array("sort_able"=>'0',"title_show"=>'&nbsp;',"field_order"=>'',"col_show"=>'action',"title_class"=>'a-center',"result_class"=>'a-left');	

		$this->session->set_userdata('productEtcBackDataSearchSession',$data_search);	
	...
	function manage_column_detail($result_list){
		...
		foreach($result_list as $item){
			...
			$item["title_action"] = '<a href="'.site_url('admin/product-'.$product_main_url.'/etc/edit/'.get_array_value($item,"aid","")).'">'.get_array_value($item,"title","-").'</a>';
			$item["publisher_name"] = get_array_value($item,"publisher_name","-");
			$item["product_main_name"] = get_array_value($item,"product_main_name","-");
			$item["cover_image_small_action"] = '<img class="border-gray" src="'.get_image(get_array_value($item,"cover_image_small_path",""),"small","").'" />';

			$status = get_array_value($item,"status","0");
			if($status == 1){
				$item["status_action"] = '<a class="btn btn-success btn-xs" title="Click to \'Inactive\' this etc." onclick="processChangeValue(\' inactive <strong>'.removeAllQuote(get_array_value($item,"title","")).'</strong>\', \'admin/product/etc\', \''.get_array_value($item,"aid","").'\', \'status=0\')"><i class="fa fa-check"></i></a>';
			}else{
				$item["status_action"] = '<a class="btn btn-danger btn-xs" title="Click to \'Active\' this etc." onclick="processChangeValue(\' active <strong>'.removeAllQuote(get_array_value($item,"title","")).'</strong>\', \'admin/product/etc\', \''.get_array_value($item,"aid","").'\', \'status=1\')"><i class="fa fa-ban"></i></a>';
			}

			$status = get_array_value($item,"is_new","0");
			if($status == 1){
				$item["is_new_action"] = '<a class="btn btn-success btn-xs" title="Click to remove new this etc." onclick="processChangeValue(\' remove new from <strong>'.removeAllQuote(get_array_value($item,"title","")).'</strong>\', \'admin/product/etc\', \''.get_array_value($item,"aid","").'\', \'is_new=0\')"><i class="fa fa-check"></i></a>';
			}else{
				$item["is_new_action"] = '<a class="btn btn-danger btn-xs" title="Click to set new this etc." onclick="processChangeValue(\' set new to <strong>'.removeAllQuote(get_array_value($item,"title","")).'</strong>\', \'admin/product/etc\', \''.get_array_value($item,"aid","").'\', \'is_new=1\')"><i class="fa fa-ban"></i></a>';
			}

			$status = get_array_value($item,"is_recommended","0");
			if($status == 1){
				$item["is_recommended_action"] = '<a class="btn btn-success btn-xs" title="Click to remove recommended this etc." onclick="processChangeValue(\' remove recommended from <strong>'.removeAllQuote(get_array_value($item,"title","")).'</strong>\', \'admin/product/etc\', \''.get_array_value($item,"aid","").'\', \'is_recommended=0\')"><i class="fa fa-check"></i></a>';
			}else{
				$item["is_recommended_action"] = '<a class="btn btn-danger btn-xs" title="Click to set recommended this etc." onclick="processChangeValue(\' set recommended to <strong>'.removeAllQuote(get_array_value($item,"title","")).'</strong>\', \'admin/product/etc\', \''.get_array_value($item,"aid","").'\', \'is_recommended=1\')"><i class="fa fa-ban"></i></a>';
			}

			$status = get_array_value($item,"is_home","0");
			if($status == 1){
				$item["is_home_action"] = '<a class="btn btn-success btn-xs" title="Click to remove this etc from home" onclick="processChangeValue(\' remove <strong>'.removeAllQuote(get_array_value($item,"title","")).'</strong> from home\', \'admin/product/etc\', \''.get_array_value($item,"aid","").' from home\', \'is_home=0\')"><i class="fa fa-check"></i></a>';
			}else{
				$item["is_home_action"] = '<a class="btn btn-danger btn-xs" title="Click to send this etc to home" onclick="processChangeValue(\' send <strong>'.removeAllQuote(get_array_value($item,"title","")).'</strong> to home\', \'admin/product/etc\', \''.get_array_value($item,"aid","").'\', \'is_home=1\')"><i class="fa fa-ban"></i></a>';
			}
			
			$item["action"] = '';
			$item["action"] .= '<a class="btn btn-success btn-xs" title="Click to add etc from this etc." onclick="processRedirect(\'admin/product-'.$product_main_url.'/etc/edit/'.get_array_value($item,"aid","").'/copy/add\')"><i class="fa fa-plus "></i></a>&nbsp;&nbsp;&nbsp;';
			
			if(is_root_admin_or_higher() || (CONST_ALLOW_DELETE_PRODUCT != "0" && is_owner_admin_or_higher())){
				$item["action"] .= '<a class="btn btn-danger btn-xs" title="Click to \'Delete\' this etc." onclick="processDelete(\''.get_array_value($item,"aid","").'\', \'admin/product/etc\', \'<strong>'.removeAllQuote(get_array_value($item,"title","")).'</strong>\')"><i class="fa fa-trash-o "></i></a>&nbsp;&nbsp;&nbsp;';
			}
	...
	function status($product_main_url="", $type="")	{
		switch($type)
		{
			...
			case md5('no-command') : 
				...
				break;
			case md5('no-etc') : 
				$this->data["message"] = set_message_error('Error occurred. Can not find this etc.');
				$this->data["js_code"] = '';
				break;
	...
	function ajax_get_etc_by_aid($sid,$aid)	{
		$this->load->model($this->main_model,"main");
