...
class Product_category_back_controller extends Product_init_controller {
	...
	function __construct(){
		...
		@define("folderName",'product/product_back/category');
		...
		$this->main_model = 'Product_category_model';
	...
	function form(){
		...
		if(CONST_CATEGORY_MODE == "2"){
			$this->load->model($this->main_model,"main");
			$this->data["all_parent_category"] = $this->main->load_all_category_parent();
		}
	...
	function add(){
		...
		$this->session->set_userdata('productCategoryBackDataSearchSession','');
	...
	function edit($aid=""){
		...
		$this->data["ref_user_section"] = $this->ref_user_section->load_all_ref_user_section_id_by_category_aid($aid);
	...
	function save(){
		...
		if($command == "_insert"){
			...
			if($aid > 0){
				$data["aid"] = $aid;
				$this->log_status('Backend : Insert product category', '['.$name.'] just added into database.', $data);
				
				$ref_user_section_arr = "";
				$ref_user_section = $this->input->get_post('ref_user_section');
				// echo "ref_user_section : ".$ref_user_section;
				if(is_var_array($ref_user_section)){
					foreach($ref_user_section as $item){
						$item = trim($item);
						if(!is_blank($item)){
							$this->load->model($this->product_category_ref_user_section_model, "ref_user_section");
							$data = array();
							$data["product_category_aid"] = $aid;
							$data["user_section_aid"] = $item;
							$data["user_owner_aid"] = $this->get_user_owner_aid_by_input();
							$this->ref_user_section->insert_or_update($data);
						}
					}
				}

				redirect('admin/product-category/status/'.md5('success'));
			}else{
				$this->log_error('Backend : Insert product category', 'Command insert_record() fail. Can not insert '.$name, $data);
			...
			if($rs){
				...
				if ($aid > 0) $this->log_status('Backend : Update product category',  '['.$name.'] has been updated.', $data);
				
				$this->load->model($this->product_category_ref_user_section_model, "ref_user_section");
				$this->ref_user_section->set_where(array("product_category_aid"=>$aid));
				$this->ref_user_section->delete_records();
				$ref_user_section_arr = "";
				$ref_user_section = $this->input->get_post('ref_user_section');
				// echo "ref_user_section : ".$ref_user_section;
				if(is_var_array($ref_user_section)){
				...
				if($save_option){
					redirect('admin/product-category/add');
				}else{
					redirect('admin/product-category/status/'.md5('success'));
				}
				return "";
			}else{
				$this->log_error('Backend : Update product category', 'Command update_record() fail. Can not update '.$name.'['.$aid.']', $data);
				$this->data["message"] = set_message_error('Sorry, the system can not save data now. Please try again or contact your administrator.');
				$this->data["js_code"] = '';
				$this->data["command"] = $command;
				$this->data["item_detail"] = $data;
			...
			$this->log_error('Backend : Product category', 'Command not found.', $data);
			redirect('admin/product-category/status/'.md5('no-command'));
			return "";
		}
	}
	...
	function check_duplicate($data="",$command=""){
		...
		$product_main_aid = get_array_value($data,"product_main_aid","");
		$user_owner_aid = get_array_value($data,"user_owner_aid","");

		$this->main->set_where(array("user_owner_aid"=>$user_owner_aid, "product_main_aid"=>$product_main_aid));
	...
	function ajax_get_main_list($sid){
		...
		$dsSession = $dataSearchSession->userdata('productCategoryBackDataSearchSession');		
		// print_r($dsSession);
		...
		$search_product_main = $this->getDataFromInput('search_product_main');
		$data_where = "";
		...
		$header_list[] = array("sort_able"=>'1',"title_show"=>'Product main',"field_order"=>'product_main_aid',"col_show"=>'product_main_name',"title_class"=>'w150 hcenter',"result_class"=>'hleft');
		...
		$this->session->set_userdata('productCategoryBackDataSearchSession',$data_search);	
	...
	function manage_column_detail($result_list){
		...
		foreach($result_list as $item){
			$item["name_action"] = '<a href="'.site_url('admin/product-category/edit/'.get_array_value($item,"aid","")).'">'.get_array_value($item,"name","-").'</a>';
			$status = get_array_value($item,"status","0");
			if($status == 1){
				$item["status_action"] = '<a class="btn btn-success btn-xs" title="Click to \'Inactive\' this book." onclick="processChangeValue(\' inactive <strong>'.removeAllQuote(get_array_value($item,"name","")).'</strong>\', \'admin/product-category\', \''.get_array_value($item,"aid","").'\', \'status=0\')"><i class="fa fa-check"></i></a>';
			}else{
				$item["status_action"] = '<a class="btn btn-danger btn-xs" title="Click to \'Active\' this book." onclick="processChangeValue(\' active <strong>'.removeAllQuote(get_array_value($item,"name","")).'</strong>\', \'admin/product-category\', \''.get_array_value($item,"aid","").'\', \'status=1\')"><i class="fa fa-ban"></i></a>';
			}

			$item["action"] = '';			
			if(is_owner_admin_or_higher()){
				$item["action"] .= '<a class="btn btn-danger btn-xs" title="Click to \'Delete\' this book." onclick="processDelete(\''.get_array_value($item,"aid","").'\', \'admin/product-category\', \'<strong>'.removeAllQuote(get_array_value($item,"name","")).'</strong>\')"><i class="fa fa-trash-o "></i></a>&nbsp;&nbsp;&nbsp;';
			}
	...
	function ajax_get_category_by_product_main($sid,$product_main_aid){
		$this->load->model($this->main_model,'main');
		$this->main->set_where('product_main_aid', $product_main_aid);
