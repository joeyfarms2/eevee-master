...
class Product_vdo_back_controller extends Product_init_controller {
	...
	function __construct(){
		...
		define("TXT_INSERT_TITLE",'Vdo form : Add new vdo');
		define("TXT_UPDATE_TITLE",'Vdo form : Edit vdo');
				
		$this->main_model = 'Vdo_model';
	}
	...
	function show($product_main_url=""){
		...
		$this->data["view_the_content"] = $this->default_theme_admin . '/' . folderName . '/vdo_list';
	...
	function form($product_main_url=""){
		define("thisAdminTabMenu",'product_vdo_'.$product_main_url);
		$this->data["title"] = DEFAULT_TITLE;
		$this->data["view_the_content"] = $this->default_theme_admin . '/' . folderName . '/vdo_form';
	...
	function add($product_main_url=""){
		...
		$this->session->set_userdata('productVdoBackDataSearchSession','');
	...
	function edit($product_main_url="", $aid=""){
		...
		if(is_var_array($parent_detail)){
			...
			$this->data["message"] = set_message_error('Cannot find the specific vdo.');
			$this->data["js_code"] = '';
			$this->show($product_main_url);
			return "";
		}
	...
	function save(){
		...
		if($command == "_update"){
			...
			$this->load->model($this->vdo_field_model,'field');
		...
		if($command == "_insert"){
			...
			if($aid > 0){
				...
				$vdo_obj = $this->save_field($product_type_aid,$product_main_aid,$aid);
				$name = get_array_value($vdo_obj,"title","-");
				$author = get_array_value($vdo_obj,"author","");
				$data["aid"] = $aid;
				$this->log_status('Backend : Insert vdo', '['.$name.']['.$aid.'] just added into database.', $data);
				$this->log_product_save($product_type_aid, $aid, $name, "I", "New vdo has been added.", $data, "vdo-add");
				$path = "uploads/".CONST_PROJECT_CODE."/vdo/".ceil($aid/100);
				$upload_path = $path."/".get_text_pad($aid,"0",CONST_ZERO_PAD_FOR_PRODUCT)."/";
				$directories = './'.$upload_path.'cover_image';
				create_directories($directories);
				$directories = './'.$upload_path.'file';
				create_directories($directories);
				$data = $this->upload_cover_image_vdo($aid, $data, $path);
				if ( !is_blank(get_array_value($data,"error_msg","")) )
				{
					// echo $data["error_msg"];
					$this->log_status('Backend : Insert vdo', 'Save vdo fail => Upload image error : '.$data["error_msg"], $data);
				...
				$data =$this->upload_parent_file($aid, $data, $path);
				if ( !is_blank(get_array_value($data,"error_msg","")) )
				{
					// echo $data["error_msg"];
					$this->log_status('Backend : Insert vdo', 'Save vdo fail => Upload vdo error : '.$data["error_msg"], $data);
					$this->data["message"] = set_message_error(get_array_value($data,"error_msg","Sorry, the system can not save data now. Please try again or contact your administrator."));
					$this->data["js_code"] = "";
					$this->data["command"] = $command;
					$this->data["parent_detail"] = $data;
				...
				$data_new["upload_path"] = $upload_path;
				$data_new["cover_image_file_type"] = get_array_value($data,"cover_image_file_type","");
				$data_new["uri"] = get_array_value($data,"uri", $uri);
				...
				// redirect('admin/product-'.$product_main_url.'/vdo/edit/'.$aid.'/copy/add');
				redirect('admin/product-'.$product_main_url.'/vdo');
			}else{
				$this->log_error('Backend : Insert vdo', 'Command insert_record() fail. Can not insert '.$name, $data);
			...
			if($rs){
				...
				if ($aid > 0) $this->log_status('Admin : Update vdo',  '['.$name.']['.$aid.'] has been updated.', $data);
				$this->log_product_save($product_type_aid, $aid, $name, "U", "General info has been updated.", $data, "vdo-update");
				$path = "uploads/".CONST_PROJECT_CODE."/vdo/".ceil($aid/100);
				$upload_path = $path."/".get_text_pad($aid,"0",CONST_ZERO_PAD_FOR_PRODUCT)."/";
				$directories = './'.$upload_path.'cover_image';
				create_directories($directories);
				...
				{
					// echo $data["error_msg"];
					$this->log_status('Backend : Update vdo', 'Save vdo fail => Upload image error : '.$data["error_msg"], $data);
					$this->data["message"] = set_message_error(get_array_value($data,"error_msg","Sorry, the system can not save data now. Please try again or contact your administrator."));
					$this->data["js_code"] = "";
					$this->data["command"] = $command;
					$this->data["parent_detail"] = $data;
				...
				$uri = get_array_value($data,"uri",$uri);
				if(!is_blank($uri)){
					$data_new["uri"] = $uri;
				}
				$this->main->set_where($data_where);
				...
				if($save_option){
					redirect('admin/product-'.$product_main_url.'/vdo/add');
				}else{
					redirect('admin/product-'.$product_main_url.'/vdo/edit/'.$aid);
					// redirect('admin/product-'.$product_main_url.'/vdo/status/'.md5('success'));
				}
				return "";
			}else{
				$this->log_error('Backend : Update vdo', 'Command update_record() fail. Can not update '.$name.'['.$aid.']', $data);
				$this->data["message"] = set_message_error('Sorry, the system can not save data now. Please try again or contact your administrator.');
				$this->data["js_code"] = '';
				$this->data["command"] = $command;
				$this->data["parent_detail"] = $data;
			...
			$this->log_error('Backend : Vdo', 'Command not found.', $data);
			redirect('admin/product-'.$product_main_url.'/vdo/status/'.md5('no-command'));
			return "";
		}
	}
	...
	function ajax_set_value($sid=""){
		...
		if ($rs){
			$this->log_status('Backend : Vdo', 'Set value : "'.$f_name.'" has been set to "'.$f_value.'" for '.$this_obj_info.'.', $objResult);
			$msg = set_message_success('Data has been saved.');
			$result_obj = array("status" => 'success',"msg" => $msg );
			echo json_encode($result_obj);
			return "";
		}else{
			$this->log_error('Backend : Vdo', 'Set value : Function main->set_value() fail. Can not set "'.$f_name.'" to "'.$f_value.'" for '.$this_obj_info.'.', $objResult);
	...
	function ajax_delete_one(){
		...
		$this->load->model($this->vdo_copy_model,'copy');
		$this->copy->set_where(array("parent_aid"=>$aid, "user_owner_aid"=>$user_owner_aid));
		$vdo_copy_list = $this->copy->load_records(false);
		if(is_var_array($vdo_copy_list)){
			foreach($vdo_copy_list as $item){
		...
		if ($rs){
			$this->log_status('Backend : Delete vdo', $this_obj_info.' has been deleted.', $objResult);
			$this->log_product_save($this->data["this_product_type_aid"], $aid, $this_obj_title, "D", "Vdo has been deleted.", $objResult, "vdo-delete");
			$msg = set_message_success($this_obj_title.' has been deleted.');
			$result_obj = array("status" => 'success',"msg" => $msg );
			echo json_encode($result_obj);
			return "";
		}else{
			$this->log_error('Backend : Delete vdo', 'Command delete_records() fail. Can not delete '.$this_obj_info.'.', $objResult);
			$msg = set_message_error('Sorry, the system can not save data now. Please try again or contact your administrator.');
			$result_obj = array("status" => 'error',"msg" => $msg );
			echo json_encode($result_obj);
			return "";
	...
	function ajax_get_main_list($sid){
		...
		$dsSession = $dataSearchSession->userdata('productVdoBackDataSearchSession');		
		// print_r($dsSession);
		...
		$this->session->set_userdata('productVdoBackDataSearchSession',$data_search);	
	...
	function manage_column_detail($result_list){
		...
		foreach($result_list as $item){
			...
			$item["title_action"] = '<a href="'.site_url('admin/product-'.$product_main_url.'/vdo/edit/'.get_array_value($item,"aid","")).'">'.get_array_value($item,"title","-").'</a>';
			$item["publisher_name"] = get_array_value($item,"publisher_name","-");
			$item["product_main_name"] = get_array_value($item,"product_main_name","-");
			$item["cover_image_small_action"] = '<img class="border-gray" src="'.get_image(get_array_value($item,"cover_image_small_path",""),"vdo-small","").'" />';

			$status = get_array_value($item,"status","0");
			if($status == 1){
				$item["status_action"] = '<a class="btn btn-success btn-xs" title="Click to \'Inactive\' this vdo." onclick="processChangeValue(\' inactive <strong>'.removeAllQuote(get_array_value($item,"title","")).'</strong>\', \'admin/product/vdo\', \''.get_array_value($item,"aid","").'\', \'status=0\')"><i class="fa fa-check"></i></a>';
			}else{
				$item["status_action"] = '<a class="btn btn-danger btn-xs" title="Click to \'Active\' this vdo." onclick="processChangeValue(\' active <strong>'.removeAllQuote(get_array_value($item,"title","")).'</strong>\', \'admin/product/vdo\', \''.get_array_value($item,"aid","").'\', \'status=1\')"><i class="fa fa-ban"></i></a>';
			}

			$status = get_array_value($item,"is_new","0");
			if($status == 1){
				$item["is_new_action"] = '<a class="btn btn-success btn-xs" title="Click to remove new this vdo." onclick="processChangeValue(\' remove new from <strong>'.removeAllQuote(get_array_value($item,"title","")).'</strong>\', \'admin/product/vdo\', \''.get_array_value($item,"aid","").'\', \'is_new=0\')"><i class="fa fa-check"></i></a>';
			}else{
				$item["is_new_action"] = '<a class="btn btn-danger btn-xs" title="Click to set new this vdo." onclick="processChangeValue(\' set new to <strong>'.removeAllQuote(get_array_value($item,"title","")).'</strong>\', \'admin/product/vdo\', \''.get_array_value($item,"aid","").'\', \'is_new=1\')"><i class="fa fa-ban"></i></a>';
			}

			$status = get_array_value($item,"is_recommended","0");
			if($status == 1){
				$item["is_recommended_action"] = '<a class="btn btn-success btn-xs" title="Click to remove recommended this vdo." onclick="processChangeValue(\' remove recommended from <strong>'.removeAllQuote(get_array_value($item,"title","")).'</strong>\', \'admin/product/vdo\', \''.get_array_value($item,"aid","").'\', \'is_recommended=0\')"><i class="fa fa-check"></i></a>';
			}else{
				$item["is_recommended_action"] = '<a class="btn btn-danger btn-xs" title="Click to set recommended this vdo." onclick="processChangeValue(\' set recommended to <strong>'.removeAllQuote(get_array_value($item,"title","")).'</strong>\', \'admin/product/vdo\', \''.get_array_value($item,"aid","").'\', \'is_recommended=1\')"><i class="fa fa-ban"></i></a>';
			}

			$status = get_array_value($item,"is_home","0");
			if($status == 1){
				$item["is_home_action"] = '<a class="btn btn-success btn-xs" title="Click to remove this vdo from home" onclick="processChangeValue(\' remove <strong>'.removeAllQuote(get_array_value($item,"title","")).'</strong> from home\', \'admin/product/vdo\', \''.get_array_value($item,"aid","").' from home\', \'is_home=0\')"><i class="fa fa-check"></i></a>';
			}else{
				$item["is_home_action"] = '<a class="btn btn-danger btn-xs" title="Click to send this vdo to home" onclick="processChangeValue(\' send <strong>'.removeAllQuote(get_array_value($item,"title","")).'</strong> to home\', \'admin/product/vdo\', \''.get_array_value($item,"aid","").'\', \'is_home=1\')"><i class="fa fa-ban"></i></a>';
			}
			
			$item["action"] = '';
			$item["action"] .= '<a class="btn btn-success btn-xs" title="Click to add vdo from this vdo." onclick="processRedirect(\'admin/product-'.$product_main_url.'/vdo/edit/'.get_array_value($item,"aid","").'/copy/add\')"><i class="fa fa-plus "></i></a>&nbsp;&nbsp;&nbsp;';
			
			if(is_root_admin_or_higher() || (CONST_ALLOW_DELETE_PRODUCT != "0" && is_owner_admin_or_higher())){
				$item["action"] .= '<a class="btn btn-danger btn-xs" title="Click to \'Delete\' this vdo." onclick="processDelete(\''.get_array_value($item,"aid","").'\', \'admin/product/vdo\', \'<strong>'.removeAllQuote(get_array_value($item,"title","")).'</strong>\')"><i class="fa fa-trash-o "></i></a>&nbsp;&nbsp;&nbsp;';
			}
	...
	function status($product_main_url="", $type="")	{
		switch($type)
		{
			...
			case md5('no-command') : 
				...
				break;
			case md5('no-vdo') : 
				$this->data["message"] = set_message_error('Error occurred. Can not find this vdo.');
				$this->data["js_code"] = '';
				break;
	...
	function ajax_get_vdo_by_aid($sid,$aid)	{
		$this->load->model($this->main_model,"main");
