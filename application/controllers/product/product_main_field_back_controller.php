...
class Product_main_field_back_controller extends Product_init_controller {
	...
	function __construct(){
		...
		$this->lang->load('product_main_field');

		define("thisAdminTabMenu",'setting');
		define("thisAdminSubMenu",'product_main_field');
		@define("folderName",'product/product_back/main_field');
		
		define("TXT_TITLE", $this->lang->line('product_main_field_title'));
		define("TXT_INSERT_TITLE", $this->lang->line('product_main_field_insert_title'));
		define("TXT_UPDATE_TITLE", $this->lang->line('product_main_field_update_title'));
				
		$this->main_model = 'Product_main_field_model';
		$this->usmarc_subfield_model = 'Usmarc_subfield_dm_model';
	}
		
	function index($product_main_aid = ""){
		$this->data["init_adv_search"] = "clear";
		$this->show($product_main_aid);
	}
	
	function show($product_main_aid="", $msg=""){
		@define("thisAction",'show');
		for_owner_admin_or_higher();
		$this->data["title"] = DEFAULT_TITLE;
		$this->data["view_the_content"] = $this->default_theme_admin . '/' . folderName . '/main_field_list';
	...
	function form($product_main_aid = ""){
		$this->data["title"] = DEFAULT_TITLE;
		$this->data["view_the_content"] = $this->default_theme_admin . '/' . folderName . '/main_field_form';
		$product_main_result = $this->check_exits_product_main_by_aid($product_main_aid,false);
		$this->data["product_main_result"] = $product_main_result;		
		$this->load->view($this->default_theme_admin.'/tpl_admin',$this->data);
	}
	
	function add($product_main_aid = ""){
		...
		$this->session->set_userdata('productMainFieldBackDataSearchSession','');

		$this->form($product_main_aid);
	}
	
	function edit($product_main_aid = "", $aid=""){
		...
		if(is_var_array($item_detail)){
			...
			$this->data["header_title"] = TXT_UPDATE_TITLE . " : " .get_array_value($product_main_result,"name","");
			$this->form($product_main_aid);
		}else{
			$this->data["message"] = set_message_error('Cannot find the specific product main field.');
			$this->show($product_main_aid);
			return "";
		}
	}
	...
	function save(){
		...
		if($command == "_insert"){
			...
			if($aid > 0){
				$data["aid"] = $aid;
				$this->log_status('Backend : Insert product main field', '['.$name.'] just added into database.', $data);
				redirect('admin/product-main-field/'.$product_main_aid.'/status/'.md5('success'));
			}else{
				$this->log_error('Backend : Insert product main field', 'Command insert_record() fail. Can not insert '.$name, $data);
			...
			if($rs){
				...
				if ($aid > 0) $this->log_status('Backend : Update product main field',  '['.$name.'] has been updated.', $data);
				...
				$this->product_field->set_where(array("product_main_field_aid"=>$aid));
				$rs = $this->product_field->update_record($tmp);

				if($save_option){
					redirect('admin/product-main-field/'.$product_main_aid.'/add');
				}else{
					redirect('admin/product-main-field/'.$product_main_aid.'/status/'.md5('success'));
				}
				return "";
			}else{
				$this->log_error('Backend : Update product main field', 'Command update_record() fail. Can not update '.$name.'['.$aid.']', $data);
				$this->data["message"] = set_message_error('Sorry, the system can not save data now. Please try again or contact your administrator.');
				$this->data["js_code"] = '';
				$this->data["command"] = $command;
				$this->data["item_detail"] = $data;
			...
			$this->log_error('Backend : Product main field', 'Command not found.', $data);
			redirect('admin/product-main-field/'.$product_main_aid.'/status/'.md5('no-command'));
			return "";
		}
	}
	...
	function ajax_set_value($sid="", $status=""){
		...
		if ($rs){
			$this->log_status('Backend : Product main field', 'Set value : "'.$f_name.'" has been set to "'.$f_value.'" for '.$this_obj_info.'.', $objResult);
			$msg = set_message_success('Data has been saved.');
			$result_obj = array("status" => 'success',"msg" => $msg );
			echo json_encode($result_obj);
			return "";
		}else{
			$this->log_error('Backend : Product main field', 'Set value : Function main->set_value() fail. Can not set "'.$f_name.'" to "'.$f_value.'" for '.$this_obj_info.'.', $objResult);
	...
	function ajax_delete_one(){
		...
		$objResult = $this->main->load_record(false);
		if(!is_var_array($objResult)){
			$msg = set_message_error('Error occurred. Can not find this product main field.');
			$result_obj = array("status" => 'error',"msg" => $msg );
			echo json_encode($result_obj);
			return "";
		}
		...
		if ($rs){
			$this->log_status('Backend : Delete product main field', $this_obj_info.' has been deleted.', $objResult);
			$msg = set_message_success($this_obj_title.' has been deleted.');
			$result_obj = array("status" => 'success',"msg" => $msg );
			echo json_encode($result_obj);
			return "";
		}else{
			$this->log_error('Backend : Delete product main field', 'Command delete_records() fail. Can not delete '.$this_obj_info.'.', $objResult);
			$msg = set_message_error('Sorry, the system can not save data now. Please try again or contact your administrator.');
			$result_obj = array("status" => 'error',"msg" => $msg );
			echo json_encode($result_obj);
			return "";
	...
	function ajax_get_main_list($sid){
		...
		$dsSession = $dataSearchSession->userdata('productMainFieldBackDataSearchSession');		
		// print_r($dsSession);
		...
		$this->session->set_userdata('productMainFieldBackDataSearchSession',$data_search);	
	...
	function manage_column_detail($result_list){
		...
		foreach($result_list as $item){
			$product_main_aid = get_array_value($item,"product_main_aid","");
			$item["name_action"] = '<a href="'.site_url('admin/product-main-field/'.$product_main_aid.'/edit/'.get_array_value($item,"aid","")).'">'.get_array_value($item,"name","-").'</a>';
			...
			if($status == 1){
				$item["status_action"] = '<a class="btn btn-success btn-xs" title="Click to \'Inactive\' this." onclick="processChangeValue(\' inactive <strong>'.removeAllQuote(get_array_value($item,"name","")).'</strong>\', \'admin/product-main-field\', \''.get_array_value($item,"aid","").'\', \'status=0\')"><i class="fa fa-check"></i></a>';
			}else{
				$item["status_action"] = '<a class="btn btn-danger btn-xs" title="Click to \'Active\' this." onclick="processChangeValue(\' active <strong>'.removeAllQuote(get_array_value($item,"name","")).'</strong>\', \'admin/product-main-field\', \''.get_array_value($item,"aid","").'\', \'status=1\')"><i class="fa fa-ban"></i></a>';
			}

			$item["action"] = '';
			$fixed_field = get_array_value($item,"fixed_field","0");
			if(is_root_admin_or_higher() || $fixed_field != "1"){
				$item["action"] .= '<a class="btn btn-danger btn-xs" title="Click to \'Delete\' this." onclick="processDelete(\''.get_array_value($item,"aid","").'\', \'admin/product-main-field\', \'<strong>'.removeAllQuote(get_array_value($item,"name","")).'</strong>\')"><i class="fa fa-trash-o "></i></a>&nbsp;&nbsp;&nbsp;';
			}
			$result[] = $item;
		}
	...
	function status($product_main_aid="", $type="")	{
		switch($type)
		{
			case md5('success') : 
				$this->data["message"] = set_message_success('Data has been saved.');
			...
			case md5('no-product-field-aid') : 
				$this->data["message"] = set_message_error('Error occurred. Can not find this product main.');
				$this->data["js_code"] = '';
				break;
			default : 
	...
	function ajax_get_field_list_by_product_main_aid($sid=""){
		...
		$product_main_aid = trim($this->input->get_post('product_main_aid'));
		$product_main_detail = $this->check_exits_product_main_by_aid($product_main_aid);

		if(is_blank($product_main_aid) || $product_main_aid <= 0){
			$result_obj = array("status" => 'error',"msg" => 'Error occurred. Product main aid is null.' );
			echo json_encode($result_obj);
			return "";
		}
		...
		$model = $this->get_product_model($product_type_aid);
		$this->load->model(get_array_value($model,"product_field_model",""),"product_field");
		if($parent_aid > 0 && $product_main_aid > 0){
	...
	function get_init_field($product_main_aid = ""){
		@define("thisAction",'add');
		for_owner_admin_or_higher();
		$this->data["command"] = '_insert';
		$this->data["header_title"] = TXT_INSERT_TITLE;
		$product_main_result = $this->check_exits_product_main_by_aid($product_main_aid,false);
		// print_r($product_main_result);
		$product_type_aid = get_array_value($product_main_result,"product_type_aid","");
		...
		switch ($product_type_aid) {
			case '1':
			case '2':
					$this->get_insert_book_init_field($product_main_aid);
			...
			case '3':
					...
					$tmp["user_owner_aid"] = getUserOwnerAid($this);
					$tmp["product_main_aid"] = $product_main_aid;
					$tmp["product_topic_main_cid"] = "0";
					$tmp["tag"] = "245";
					$tmp["subfield_cd"] = "a";
					$tmp["description"] = NULL;
					$tmp["is_required"] = "1";
					$tmp["input_type"] = "textbox";
					$tmp["status"] = "1";
					$tmp["weight"] = "-1";
					$tmp["fixed_field"] = "1";
					$this->main->insert_record($tmp);
		...
		redirect('admin/product-main-field/'.$product_main_aid);
	}

	function get_insert_book_init_field($product_main_aid){
		...
		$this->load->model($this->main_model,"main");
					...
					if($product_main_aid == "2" || $product_main_aid == "4"){
					...
					if($product_main_aid == "1" || $product_main_aid == "2" || $product_main_aid == "8"){
						...
						$tmp["tag"] = "050";
						$tmp["subfield_cd"] = "b";
						$tmp["description"] = "ฉบับหรือเล่มที่";
						$tmp["is_required"] = "0";
						$tmp["input_type"] = "textbox";
						$tmp["status"] = "1";
						$tmp["weight"] = "0";
					...
					$tmp["tag"] = "260";
					$tmp["subfield_cd"] = "b";
					$tmp["description"] = "ชื่อสำนักพิมพ์";
					$tmp["is_required"] = "0";
					$tmp["input_type"] = "textbox";
					$tmp["status"] = "1";
					$tmp["weight"] = "0";
					...
					if ($product_main_aid == "7") {
						...
						$tmp["tag"] = "856";
						$tmp["subfield_cd"] = "";
						$tmp["description"] = "ลิงค์เชื่อมโยง";
						$tmp["is_required"] = "0";
						$tmp["input_type"] = "textbox";
						$tmp["status"] = "1";
						$tmp["weight"] = "999";
