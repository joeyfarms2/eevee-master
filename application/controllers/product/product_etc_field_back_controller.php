...
class Product_etc_field_back_controller extends Product_init_controller {
	...
	function __construct(){
		...
		$this->main_model = 'Etc_field_model';
	...
	function show($product_main_url="", $parent_aid="", $msg=""){
		...
		$this->data["view_the_content"] = $this->default_theme_admin . '/' . folderName . '/etc_field_list';
	...
	function form($product_main_url="", $parent_aid=""){
		define("thisAdminTabMenu",'product_etc_'.$product_main_url);
		$this->data["title"] = DEFAULT_TITLE;
		$this->data["view_the_content"] = $this->default_theme_admin . '/' . folderName . '/etc_field_form';
	...
	function edit($product_main_url="", $parent_aid="", $sequence=""){
		...
		if(is_var_array($field_item_detail)){
			...
			$this->data["message"] = set_message_error('Cannot find the specific etc field.');
			$this->data["js_code"] = "";
	...
	function save(){
		...
		$product_type_field_aid = $this->input->get_post('product_type_field_aid');
		...
		if($command == "_insert"){
			...
			$data["parent_aid"] = $parent_aid;
			$data["product_type_field_aid"] = "0";
			
			$aid = $this->main->insert_record($data);
			// if($aid){
				$this->log_status('Backend : Insert etc field', '[name='.$name.'][sequence='.$sequence.'][field_data='.$field_data.'] of ['.$title.']['.$parent_aid.'] just added into database.', $data);
				$this->log_product_save($product_type_aid, $parent_aid, $title, "U", "Biblio field [$name] has been added", $data, "field-add");
				redirect('admin/product-'.$product_main_url.'/etc/edit/'.$parent_aid.'/field/status/'.md5('success'));
			// }else{
				// $this->log_error('Backend : Insert etc', 'Command insert_record() fail. Can not insert '.$name);
			...
			if($rs){
				...
				$this->log_status('Admin : Update etc field',  '[name='.$name.'][sequence='.$sequence.'][field_data='.$field_data.'] of ['.$title.']['.$parent_aid.'] has been updated.', $data);
				$this->log_product_save($product_type_aid, $parent_aid, $title, "U", "Biblio field [$name] has been updated", $data, "field-update");
				if($save_option){
					redirect('admin/product-'.$product_main_url.'/etc/edit/'.$parent_aid.'/field/add');
				}else{
					redirect('admin/product-'.$product_main_url.'/etc/edit/'.$parent_aid.'/field/status/'.md5('success'));
				}
				return "";
			}else{
				$this->log_error('Backend : Update etc field', 'Command update_record() fail. Can not update [name='.$name.'][sequence='.$sequence.'][field_data='.$field_data.'] of ['.$title.']['.$parent_aid.']', $data);
				$this->data["message"] = set_message_error('Sorry, The system can not insert data right now.<BR>Please try again later or contact administrator to solve the problem.');
				$this->data["js_code"] = '';
				$this->data["command"] = $command;
				$this->data["field_item_detail"] = $data;
			...
			$this->log_error('Backend : Etc field', 'Command not found.');
			redirect('admin/product-'.$product_main_url.'/etc/edit/'.$parent_aid.'field/status/'.md5('no-command'));
			return "";
		}
	}
	...
	function ajax_delete_one(){
		...
		if(!is_var_array($objResult)){
			$msg = set_message_error('Error occurred. Can not find this etc field.');
			$result_obj = array("status" => 'error',"msg" => $msg );
			echo json_encode($result_obj);
			return "";
		}
		...
		if ($rs){
			$this->log_status('Backend : Delete biblio field', '[name='.$this_name.'][sequence='.$sequence.'][field_data='.$field_data.'] of ['.$title.']['.$parent_aid.'] has been deleted.', $objResult);
			$this->log_product_save($product_type_aid, $parent_aid, $title, "U", "Biblio field [$this_name] has been deleted", $objResult, "field-delete");
			$msg = set_message_success($this_name.' has been deleted.');
			$result_obj = array("status" => 'success',"msg" => $msg );
			echo json_encode($result_obj);
		}else{
			$this->log_error('Backend : Delete biblio field', 'Command delete_records() fail. Can not delete [name='.$this_name.'][sequence='.$sequence.'][field_data='.$field_data.'] of ['.$title.']['.$parent_aid.'].', $objResult);
	...
	function ajax_get_main_list($sid){
		...
		$order_by_option = $this->get_order_by_info($search_order_by,'product_type_field.weight asc, product_type_field.created_date ASC');
	...
	function manage_column_detail($result_list, $product_main_url="0"){
		...
		foreach($result_list as $item){
			...
			$item["name_action"] = '<a href="'.site_url('admin/product-'.$product_main_url.'/etc/edit/'.$parent_aid.'/field/edit/'.get_array_value($item,"sequence","")).'">'.get_array_value($item,"product_type_field_name","-").'</a>';
			// $item["cid"] = get_text_pad(get_array_value($item,"aid","0"));
			$item["action"] = '';

			$product_type_field_aid = get_array_value($item,"product_type_field_aid","0");
			if($product_type_field_aid <= 0){
				$item["action"] .= '<a class="btn btn-danger btn-xs" title="Click to \'Delete\' this field" onclick="processDeleteField(\''.get_array_value($item,"parent_aid","").'\', \''.get_array_value($item,"sequence","").'\', \'admin/product/etc-field\', \''.get_array_value($item,"name","").'\')"><i class="fa fa-trash-o "></i></a>&nbsp;&nbsp;&nbsp;';
			}
