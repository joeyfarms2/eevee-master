...
class Product_vdo_field_back_controller extends Product_init_controller {
	...
	function __construct(){
		...
		$this->main_model = 'Vdo_field_model';
	...
	function show($product_main_url="", $parent_aid="", $msg=""){
		...
		$this->data["view_the_content"] = $this->default_theme_admin . '/' . folderName . '/vdo_field_list';
	...
	function form($product_main_url="", $parent_aid=""){
		define("thisAdminTabMenu",'product_vdo_'.$product_main_url);
		$this->data["title"] = DEFAULT_TITLE;
		$this->data["view_the_content"] = $this->default_theme_admin . '/' . folderName . '/vdo_field_form';
	...
	function edit($product_main_url="", $parent_aid="", $sequence=""){
		...
		if(is_var_array($field_item_detail)){
			...
			$this->data["message"] = set_message_error('Cannot find the specific vdo field.');
			$this->data["js_code"] = "";
	...
	function save(){
		...
		if($command == "_insert"){
			...
			// if($aid){
				$this->log_status('Backend : Insert vdo field', '[name='.$name.'][sequence='.$sequence.'][field_data='.$field_data.'] of ['.$title.']['.$parent_aid.'] just added into database.', $data);
				$this->log_product_save($product_type_aid, $parent_aid, $title, "U", "Field [$name] has been added", $data, "field-add");
				redirect('admin/product-'.$product_main_url.'/vdo/edit/'.$parent_aid.'/field/status/'.md5('success'));
			// }else{
				// $this->log_error('Backend : Insert vdo', 'Command insert_record() fail. Can not insert '.$name);
			...
			if($rs){
				...
				$this->log_status('Admin : Update vdo field',  '[name='.$name.'][sequence='.$sequence.'][field_data='.$field_data.'] of ['.$title.']['.$parent_aid.'] has been updated.', $data);
				$this->log_product_save($product_type_aid, $parent_aid, $title, "U", "Field [$name] has been updated", $data, "field-update");
				if($save_option){
					redirect('admin/product-'.$product_main_url.'/vdo/edit/'.$parent_aid.'/field/add');
				}else{
					redirect('admin/product-'.$product_main_url.'/vdo/edit/'.$parent_aid.'/field/status/'.md5('success'));
				}
				return "";
			}else{
				$this->log_error('Backend : Update vdo field', 'Command update_record() fail. Can not update [name='.$name.'][sequence='.$sequence.'][field_data='.$field_data.'] of ['.$title.']['.$parent_aid.']', $data);
				$this->data["message"] = set_message_error('Sorry, The system can not insert data right now.<BR>Please try again later or contact administrator to solve the problem.');
				$this->data["js_code"] = '';
				$this->data["command"] = $command;
				$this->data["field_item_detail"] = $data;
			...
			$this->log_error('Backend : Vdo field', 'Command not found.');
			redirect('admin/product-'.$product_main_url.'/vdo/edit/'.$parent_aid.'field/status/'.md5('no-command'));
			return "";
		}
	}
	...
	function ajax_delete_one(){
		...
		if(!is_var_array($objResult)){
			$msg = set_message_error('Error occurred. Can not find this vdo field.');
			$result_obj = array("status" => 'error',"msg" => $msg );
			echo json_encode($result_obj);
			return "";
		}
	...
	function manage_column_detail($result_list, $product_main_url="0"){
		...
		foreach($result_list as $item){
			...
			$item["name_action"] = '<a href="'.site_url('admin/product-'.$product_main_url.'/vdo/edit/'.$parent_aid.'/field/edit/'.get_array_value($item,"sequence","")).'">'.get_array_value($item,"product_main_field_name","-").'</a>';
			...
			if($product_main_field_aid <= 0){
				$item["action"] .= '<a class="btn btn-danger btn-xs" title="Click to \'Delete\' this field" onclick="processDeleteField(\''.get_array_value($item,"parent_aid","").'\', \''.get_array_value($item,"sequence","").'\', \'admin/product/vdo-field\', \''.get_array_value($item,"name","").'\')"><i class="fa fa-trash-o "></i></a>&nbsp;&nbsp;&nbsp;';
			}
