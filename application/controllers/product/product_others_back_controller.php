...
class Product_others_back_controller extends Product_init_controller {
	...
	function __construct(){
		...
		define("TXT_INSERT_TITLE",'Others form : Add new others');
		define("TXT_UPDATE_TITLE",'Others form : Edit others');
				
		$this->main_model = 'Others_model';
	}
	...
	function show($product_main_url=""){
		...
		$this->data["view_the_content"] = $this->default_theme_admin . '/' . folderName . '/others_list';
	...
	function form($product_main_url=""){
		define("thisAdminTabMenu",'product_others_'.$product_main_url);
		$this->data["title"] = DEFAULT_TITLE;
		$this->data["view_the_content"] = $this->default_theme_admin . '/' . folderName . '/others_form';
	...
	function add($product_main_url=""){
		...
		$this->session->set_userdata('productOthersBackDataSearchSession','');
	...
	function edit($product_main_url="", $aid=""){
		...
		if(is_var_array($parent_detail)){
			...
			$this->data["message"] = set_message_error('Cannot find the specific others.');
			$this->data["js_code"] = '';
			$this->show($product_main_url);
			return "";
		}
	...
	function save(){
		...
		if($command == "_update"){
			...
			$this->load->model($this->others_field_model,'field');
		...
		if($command == "_insert"){
			...
			if($aid > 0){
				...
				$others_obj = $this->save_field($product_type_aid,$product_main_aid,$aid);
				$name = get_array_value($others_obj,"title","-");
				$author = get_array_value($others_obj,"author","");
				$data["aid"] = $aid;
				$this->log_status('Backend : Insert others', '['.$name.']['.$aid.'] just added into database.', $data);
				$this->log_product_save($product_type_aid, $aid, $name, "I", "New others has been added.", $data, "others-add");
				$path = "uploads/".CONST_PROJECT_CODE."/others/".ceil($aid/100);
				$data = $this->upload_cover_image($aid, $data, $path);
				if ( !is_blank(get_array_value($data,"error_msg","")) )
				{
					// echo $data["error_msg"];
					$this->log_status('Backend : Insert others', 'Save others fail => Upload image error : '.$data["error_msg"], $data);
				...
				redirect('admin/product-'.$product_main_url.'/others/edit/'.$aid.'/copy/add');
			}else{
				$this->log_error('Backend : Insert others', 'Command insert_record() fail. Can not insert '.$name, $data);
			...
			if($rs){
				...
				if ($aid > 0) $this->log_status('Admin : Update others',  '['.$name.']['.$aid.'] has been updated.', $data);
				$this->log_product_save($product_type_aid, $aid, $name, "U", "General info has been updated.", $data, "others-update");
				$path = "uploads/".CONST_PROJECT_CODE."/others/".ceil($aid/100);
				$data = $this->upload_cover_image($aid, $data, $path);
				if ( !is_blank(get_array_value($data,"error_msg","")) )
				{
					// echo $data["error_msg"];
					$this->log_status('Backend : Update others', 'Save others fail => Upload image error : '.$data["error_msg"], $data);
					$this->data["message"] = set_message_error(get_array_value($data,"error_msg","Sorry, the system can not save data now. Please try again or contact your administrator."));
					$this->data["js_code"] = "";
					$this->data["command"] = $command;
					$this->data["parent_detail"] = $data;
				...
				if($save_option){
					redirect('admin/product-'.$product_main_url.'/others/add');
				}else{
					redirect('admin/product-'.$product_main_url.'/others/edit/'.$aid);
					// redirect('admin/product-'.$product_main_url.'/others/status/'.md5('success'));
				}
				return "";
			}else{
				$this->log_error('Backend : Update others', 'Command update_record() fail. Can not update '.$name.'['.$aid.']', $data);
				$this->data["message"] = set_message_error('Sorry, the system can not save data now. Please try again or contact your administrator.');
				$this->data["js_code"] = '';
				$this->data["command"] = $command;
				$this->data["parent_detail"] = $data;
			...
			$this->log_error('Backend : Others', 'Command not found.', $data);
			redirect('admin/product-'.$product_main_url.'/others/status/'.md5('no-command'));
			return "";
		}
	}
	...
	function ajax_set_value($sid=""){
		...
		if ($rs){
			$this->log_status('Backend : Others', 'Set value : "'.$f_name.'" has been set to "'.$f_value.'" for '.$this_obj_info.'.', $objResult);
			$msg = set_message_success('Data has been saved.');
			$result_obj = array("status" => 'success',"msg" => $msg );
			echo json_encode($result_obj);
			return "";
		}else{
			$this->log_error('Backend : Others', 'Set value : Function main->set_value() fail. Can not set "'.$f_name.'" to "'.$f_value.'" for '.$this_obj_info.'.', $objResult);
	...
	function ajax_delete_one(){
		...
		$this->load->model($this->others_copy_model,'copy');
		$this->copy->set_where(array("parent_aid"=>$aid, "user_owner_aid"=>$user_owner_aid));
		$others_copy_list = $this->copy->load_records(false);
		if(is_var_array($others_copy_list)){
			foreach($others_copy_list as $item){
		...
		if ($rs){
			$this->log_status('Backend : Delete others', $this_obj_info.' has been deleted.', $objResult);
			$this->log_product_save($this->data["this_product_type_aid"], $aid, $this_obj_title, "D", "Others has been deleted.", $objResult, "others-delete");
	...
	function ajax_get_main_list($sid){
		...
		$dsSession = $dataSearchSession->userdata('productOthersBackDataSearchSession');		
		// print_r($dsSession);
		...
		$header_list[] = array("sort_able"=>'1',"title_show"=>'Name',"field_order"=>'title',"col_show"=>'title_action',"title_class"=>'w150 a-center',"result_class"=>'a-left');
		...
		$this->session->set_userdata('productOthersBackDataSearchSession',$data_search);	
	...
	function manage_column_detail($result_list){
		...
		foreach($result_list as $item){
			...
			$item["title_action"] = '<a href="'.site_url('admin/product-'.$product_main_url.'/others/edit/'.get_array_value($item,"aid","")).'">'.get_array_value($item,"title","-").'</a>';
			...
			if($status == 1){
				$item["status_action"] = '<a class="btn btn-success btn-xs" title="Click to \'Inactive\' this others." onclick="processChangeValue(\' inactive <strong>'.removeAllQuote(get_array_value($item,"title","")).'</strong>\', \'admin/product/others\', \''.get_array_value($item,"aid","").'\', \'status=0\')"><i class="fa fa-check"></i></a>';
			}else{
				$item["status_action"] = '<a class="btn btn-danger btn-xs" title="Click to \'Active\' this others." onclick="processChangeValue(\' active <strong>'.removeAllQuote(get_array_value($item,"title","")).'</strong>\', \'admin/product/others\', \''.get_array_value($item,"aid","").'\', \'status=1\')"><i class="fa fa-ban"></i></a>';
			}

			$status = get_array_value($item,"is_new","0");
			if($status == 1){
				$item["is_new_action"] = '<a class="btn btn-success btn-xs" title="Click to remove new this others." onclick="processChangeValue(\' remove new from <strong>'.removeAllQuote(get_array_value($item,"title","")).'</strong>\', \'admin/product/others\', \''.get_array_value($item,"aid","").'\', \'is_new=0\')"><i class="fa fa-check"></i></a>';
			}else{
				$item["is_new_action"] = '<a class="btn btn-danger btn-xs" title="Click to set new this others." onclick="processChangeValue(\' set new to <strong>'.removeAllQuote(get_array_value($item,"title","")).'</strong>\', \'admin/product/others\', \''.get_array_value($item,"aid","").'\', \'is_new=1\')"><i class="fa fa-ban"></i></a>';
			}

			$status = get_array_value($item,"is_recommended","0");
			if($status == 1){
				$item["is_recommended_action"] = '<a class="btn btn-success btn-xs" title="Click to remove recommended this others." onclick="processChangeValue(\' remove recommended from <strong>'.removeAllQuote(get_array_value($item,"title","")).'</strong>\', \'admin/product/others\', \''.get_array_value($item,"aid","").'\', \'is_recommended=0\')"><i class="fa fa-check"></i></a>';
			}else{
				$item["is_recommended_action"] = '<a class="btn btn-danger btn-xs" title="Click to set recommended this others." onclick="processChangeValue(\' set recommended to <strong>'.removeAllQuote(get_array_value($item,"title","")).'</strong>\', \'admin/product/others\', \''.get_array_value($item,"aid","").'\', \'is_recommended=1\')"><i class="fa fa-ban"></i></a>';
			}

			$status = get_array_value($item,"is_home","0");
			if($status == 1){
				$item["is_home_action"] = '<a class="btn btn-success btn-xs" title="Click to remove this others from home" onclick="processChangeValue(\' remove <strong>'.removeAllQuote(get_array_value($item,"title","")).'</strong> from home\', \'admin/product/others\', \''.get_array_value($item,"aid","").' from home\', \'is_home=0\')"><i class="fa fa-check"></i></a>';
			}else{
				$item["is_home_action"] = '<a class="btn btn-danger btn-xs" title="Click to send this others to home" onclick="processChangeValue(\' send <strong>'.removeAllQuote(get_array_value($item,"title","")).'</strong> to home\', \'admin/product/others\', \''.get_array_value($item,"aid","").'\', \'is_home=1\')"><i class="fa fa-ban"></i></a>';
			}
			
			$item["action"] = '';
			$item["action"] .= '<a class="btn btn-success btn-xs" title="Click to add others from this others." onclick="processRedirect(\'admin/product-'.$product_main_url.'/others/edit/'.get_array_value($item,"aid","").'/copy/add\')"><i class="fa fa-plus "></i></a>&nbsp;&nbsp;&nbsp;';
			
			if(is_root_admin_or_higher() || (CONST_ALLOW_DELETE_PRODUCT != "0" && is_owner_admin_or_higher())){
				$item["action"] .= '<a class="btn btn-danger btn-xs" title="Click to \'Delete\' this others." onclick="processDelete(\''.get_array_value($item,"aid","").'\', \'admin/product/others\', \'<strong>'.removeAllQuote(get_array_value($item,"title","")).'</strong>\')"><i class="fa fa-trash-o "></i></a>&nbsp;&nbsp;&nbsp;';
			}
	...
	function status($product_main_url="", $type="")	{
		switch($type)
		{
			...
			case md5('no-command') : 
				...
				break;
			case md5('no-others') : 
				$this->data["message"] = set_message_error('Error occurred. Can not find this others.');
				$this->data["js_code"] = '';
				break;
	...
	function ajax_get_others_by_aid($sid,$aid)	{
		$this->load->model($this->main_model,"main");
