...
class Topread_back_controller extends Product_init_controller {
    ...
    function create_table_popular_categories() {
        ...
        //echo $this->db->last_query();
        $this->db->flush_cache();
        $sql2 ="CREATE TABLE `view_popular_categories` (`aid` int(10) unsigned NOT NULL auto_increment,`product_main_name` char(100),`name` char(100),`total` int(10) ,PRIMARY KEY  (`aid`))ENGINE=MyISAM DEFAULT CHARSET=utf8; ";
        $qs2 = $this->db->query($sql2);
        //echo $this->db->last_query();
        $this->db->flush_cache();
    }
    ...
    function get_top_most_popular_categories() {
        ...
        $this->session->set_userdata('TopMostPopularCategoriesBackDataSearchSession',$data_search);
        ...
        if (is_var_array($master_category)) {
            foreach ($master_category as $row) {
                ...
                $total = 0;
                if ($product_main_aid == "2") { //แม็กกาซีน
                ...
                if (is_var_array($result)) {
                    $this->db->flush_cache();
                    $this->load->model($this->transaction_model, "transaction");
                    if ($product_main_aid == "1" OR $product_main_aid == "2") { //หนังสือเล่ม
                        if ($product_main_aid == "1") { //หนังสือเล่ม
    ...
    function get_top_most_popular_item() {
        ...
        if(! $product_main_aid) $product_main_aid= 1;
	...
	function get_not_borrow_item() {
        $this->load->model($this->transaction_model, "transaction");
        $product_main_aid = ($this->input->get_post('type'))? $this->input->get_post('type'):1;
	...
	function get_new_item() {
        ...
        // echo $borrowing_date_from."<br/>";
        // echo $borrowing_date_to."<br/>";

        $data_array = array();
        if($product_main_aid){
            if ($product_main_aid == "2" OR $product_main_aid == "4") { 
                    $this->load->model($this->magazine_copy_model, "magazine_copy");
                    $this->magazine_copy->set_where('magazine_copy.status', 1);
                    $this->db->where_not_in('magazine_copy.aid', $ignore);
                    $this->magazine_copy->set_where("product_main_aid",$product_main_aid);
