...
class Banner_back_controller extends Project_init_controller {
	...
	function __construct(){
		...
		define("thisAdminSubMenu",'banner');
		@define("folderName",'banner/banner_back/banner');
		
		define("TXT_TITLE",'Banner management');
		define("TXT_INSERT_TITLE",'Banner management : Add new banner');
		define("TXT_UPDATE_TITLE",'Banner management : Edit banner');
				
		$this->main_model = 'Banner_model';		
	}
	...
	function show($msg=""){
		...
		$this->data["view_the_content"] = $this->default_theme_admin . '/' . folderName . '/banner_list';
	...
	function form(){
		$this->data["title"] = DEFAULT_TITLE;
		$this->data["view_the_content"] = $this->default_theme_admin . '/' . folderName . '/banner_form';
		$this->load->view($this->default_theme_admin.'/tpl_admin',$this->data);
	}
	...
	function add(){
		...
		$this->session->set_userdata('bannerBackDataSearchSession','');
	...
	function edit($aid=""){
		...
		if(is_var_array($item_detail)){
			...
			$this->data["message"] = set_message_error('Cannot find the specific banner.');
			$this->data["js_code"] = "$('#title').focus();";
			$this->show();
			return "";
		}
	...
	function save(){
		...
		$upload_base_path = "./uploads/".CONST_PROJECT_CODE."/banner";
		$upload_base_path_db = "uploads/".CONST_PROJECT_CODE."/banner";
		...
		if($command == "_update"){
			...
			if(!is_var_array($itemResult)){
				$this->data["message"] = set_message_error("Banner not found.");
				$this->data["js_code"] = "";
		...
		if(is_blank($cid)){
			...
			}while( $this->isBannerCodeExits($cid) );
		}
		$data["cid"] = trim($cid);
		...
		if( !is_blank(get_array_value($_FILES,"cover_image","")) && !is_blank(get_array_value($_FILES["cover_image"],"name","")) ){
			...
			$result_image_thumb = upload_image("cover_image",$upload_path,$new_file_name_thumb,CONST_ALLOW_FILE_SIZE_FOR_IMAGE,0,415,99,1);
			...
			if ( !is_blank(get_array_value($result_image_thumb,"error_msg","")) )
			{
				...
				$this->log_error('Admin : Banner', 'Add new banner fail => Upload image error : '.$result_image_thumb["error_msg"]);
		...
		if($command == "_insert"){
			...
			if($aid > 0){
				$data["aid"] = $aid;
				$this->log_status('Backend : Insert banner', 'New banner just added into database.', $data);
				redirect('admin/banner/status/'.md5('success'));
			}else{
				$this->log_error('Backend : Insert banner', 'Command insert_record() fail. Can not insert new banner', $data);
			...
			if($rs){
				...
				if ($aid > 0) $this->log_status('Backend : Update banner',  'Banner has been updated.', $data);
				if($save_option){
				...
				$this->log_error('Backend : Update banner', 'Command update_record() fail. Can not update banner['.$aid.']', $data);
				$this->data["message"] = set_message_error('Sorry, the system can not save data now. Please try again or contact your administrator.');
				$this->data["js_code"] = '';
				$this->data["command"] = $command;
				$this->data["item_detail"] = $data;
			...
			$this->log_error('Backend : Banner', 'Command not found.', $data);
			redirect('admin/banner/status/'.md5('no-command'));
			return "";
		}
	}
	...
	function ajax_set_value($sid="", $status=""){
		...
		if(!is_var_array($objResult)){
			$msg = set_message_error('Error occurred. Can not find this banner.');
			$result_obj = array("status" => 'error',"msg" => $msg );
			echo json_encode($result_obj);
			return "";
		}
		...
		if ($rs){
			$this->log_status('Backend : Banner', 'Set value : "'.$f_name.'" has been set to "'.$f_value.'" for '.$this_obj_info.'.', $objResult);
			$msg = set_message_success('Data has been saved.');
			$result_obj = array("status" => 'success',"msg" => $msg );
			echo json_encode($result_obj);
			return "";
		}else{
			$this->log_error('Backend : Banner', 'Set value : Function main->set_value() fail. Can not set "'.$f_name.'" to "'.$f_value.'" for '.$this_obj_info.'.', $objResult);
	...
	function ajax_delete_one(){
		...
		if ($rs){
			$this->log_status('Backend : Delete banner', $this_obj_info.' has been deleted.', $objResult);
			$msg = set_message_success($this_obj_title.' has been deleted.');
			$result_obj = array("status" => 'success',"msg" => $msg );
			echo json_encode($result_obj);
			return "";
		}else{
			$this->log_error('Backend : Delete banner', 'Command delete_records() fail. Can not delete '.$this_obj_info.'.', $objResult);
			$msg = set_message_error('Sorry, the system can not save data now. Please try again or contact your administrator.');
			$result_obj = array("status" => 'error',"msg" => $msg );
			echo json_encode($result_obj);
			return "";
	...
	function ajax_get_main_list($sid){
		...
		$dsSession = $dataSearchSession->userdata('bannerBackDataSearchSession');		
		// print_r($dsSession);
		...
		$this->session->set_userdata('bannerBackDataSearchSession',$data_search);	
	...
	function manage_column_detail($result_list){
		...
		foreach($result_list as $item){
			$item["title_action"] = '<a href="'.site_url('admin/banner/edit/'.get_array_value($item,"aid","")).'">'.get_array_value($item,"title","((no title)))").'</a>';
			$status = get_array_value($item,"status","0");
			if($status == 1){
				$item["status_action"] = '<a class="btn btn-success btn-xs" title="Click to \'Inactive\' this banner." onclick="processChangeValue(\' inactive <strong>'.removeAllQuote(get_array_value($item,"title","")).'</strong>\', \'admin/banner\', \''.get_array_value($item,"aid","").'\', \'status=0\')"><i class="fa fa-check"></i></a>';
			}else{
				$item["status_action"] = '<a class="btn btn-danger btn-xs" title="Click to \'Active\' this banner." onclick="processChangeValue(\' active <strong>'.removeAllQuote(get_array_value($item,"title","")).'</strong>\', \'admin/banner\', \''.get_array_value($item,"aid","").'\', \'status=1\')"><i class="fa fa-ban"></i></a>';
			}

			$item["action"] = '';			
			if(is_staff_or_higher()){
				$item["action"] .= '<a class="btn btn-danger btn-xs" title="Click to \'Delete\' this banner." onclick="processDelete(\''.get_array_value($item,"aid","").'\', \'admin/banner\', \'<strong>'.removeAllQuote(get_array_value($item,"title","")).'</strong>\')"><i class="fa fa-trash-o "></i></a>&nbsp;&nbsp;&nbsp;';
			}
	...
	function isBannerCodeExits($cid){
		$this->load->model($this->main_model,"main");
