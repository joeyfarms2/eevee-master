...
class News_upload_back_controller extends Initcontroller {
	...
	function __construct(){
		...
		define("thisAdminSubMenu",'news_upload');
		@define("folderName",'news/');
		
		define("TXT_TITLE",'News upload management');
		define("TXT_INSERT_TITLE",'Add new news upload');
		define("TXT_UPDATE_TITLE",'Edit news upload');
				
		$this->main_model = 'News_upload_model';
	...
	function show($msg=""){
		...
		$this->data["view_the_content"] = $this->default_theme_admin . '/news/news_upload_list';
	...
	function save(){
		...
		$news_upload_aid = $this->input->get_post('aid');
		$this->load->model($this->main_model,'main');
		$this->main->set_where(array("aid"=>$news_upload_aid,"status"=>"1"));
		$objResult = $this->main->load_record(false);
		if(!is_var_array($objResult)){
			redirect('admin/news-upload/status/'.md5('no-news-upload'));
		}
		
		$data["news_upload_aid"] = get_array_value($objResult,"aid","0");
		$data["user_owner_aid"] = get_array_value($objResult,"user_owner_aid","1");
		$data["title"] = get_array_value($objResult,"title","");
		$data["weight"] = "0";
		$data["news_main_aid"] = get_array_value($objResult,"news_main_aid","");
		...
		$aid = $this->news->insert_record($data);
		if($aid > 0){
			$this->load->model($this->main_model,'main');
			$data = "";
			$data["status"] = "2";
			$data_where["aid"] = $news_upload_aid;
			$this->main->set_where($data_where);
			$rs = $this->main->update_record($data, $data_where);
		
			$this->log_status('Backend : Insert news fron news upload', '[news aid='.$aid.'] just added into database.');
			redirect('admin/news/edit/'.$aid);
		}else{
			$this->log_error('Backend : Insert news', 'Command insert_record() fail. Can not insert.');
			redirect('admin/news-upload/status/'.md5('insert-error'));
			return "";
		}
	...
	function ajax_set_status($sid="", $status=""){
		...
		$objResult = $this->main->load_record(false);
		if(!is_var_array($objResult)){
			$result_obj = array("status" => 'error',"msg" => 'Error occurred. Can not find this news.' );
			echo json_encode($result_obj);
			return "";
		}
		...
		if ($rs){
			if($status == 1){
				$this->log_status('Backend : News status', $this_name.'['.$aid.'] was active.');
				$result_obj = array("status" => 'success',"msg" => $this_name.' was active.' );
				echo json_encode($result_obj);
				return "";
			}else{
				$this->log_status('Backend : News status', $this_name.'['.$aid.'] was inactive.');
				$result_obj = array("status" => 'success',"msg" => $this_name.' was inactive.' );
				echo json_encode($result_obj);
				return "";
			}
		}else{
			$this->log_error('Backend : News status', 'Function main->set_status() fail. Can not set status to '.$this_name.'['.$aid.'].');
	...
	function ajax_get_main_list($sid){
		...
		if(is_staff()){
			$this->main->set_where(array("news_main_aid"=>getUserLoginPublisherAid($this)));
		}
		...
		*/
		...
		$header_list[] = array("sort_able"=>'1',"title_show"=>'Provice',"field_order"=>'*news_main_name',"col_show"=>'news_main_name',"title_class"=>'w100 hcenter',"result_class"=>'hleft');
	...
	function manage_column_detail($result_list){
		...
		foreach($result_list as $item){
			$item["name_action"] = '<a href="'.site_url('admin/news-upload/detail/'.get_array_value($item,"aid","")).'">'.get_text_pad(get_array_value($item,"aid","0"),0,8).'</a>';
			
			$status = get_array_value($item,"status","0");
			if($status == 1){
				$item["status_action"] = '<img src="'.CSS_PATH.'dandelion/images/icons/color/accept.png" class="button" title="Click to \'Inctive\' this news" onclick="processChangeValue(\''.get_array_value($item,"aid","").'\', \'admin/news\', \' inactive '.get_array_value($item,"title","").'\', \'status=0\')">';
			}else{
				$item["status_action"] = '<img src="'.CSS_PATH.'dandelion/images/icons/color/cross.png" class="button" title="Click to \'Active\' this news" onclick="processChangeValue(\''.get_array_value($item,"aid","").'\', \'admin/news\', \' active '.get_array_value($item,"title","").'\', \'status=1\')">';
			}
			
			$status = get_array_value($item,"is_highlight","0");
			if($status == 1){
				$item['is_highlight_action'] = '<img src="'.CSS_PATH.'dandelion/images/icons/color/accept.png" class="button" title="Click to remove highlight form this news" onclick="processChangeHighlight(\''.get_array_value($item,"aid","").'\', \''.removeAllQuote(get_array_value($item,"title","")).'\', 0)">';
			}else{
				$item['is_highlight_action'] = '<img src="'.CSS_PATH.'dandelion/images/icons/color/cross.png" class="button" title="Click to set highlight this news" onclick="processChangeHighlight(\''.get_array_value($item,"aid","").'\', \''.removeAllQuote(get_array_value($item,"title","")).'\', 1)">';
			}
			
			$status = get_array_value($item,"is_recommended","0");
			if($status == 1){
				$item['is_recommended_action'] = '<img src="'.CSS_PATH.'dandelion/images/icons/color/accept.png" class="button" title="Click to remove recommended form this news" onclick="processChangeRecommended(\''.get_array_value($item,"aid","").'\', \''.removeAllQuote(get_array_value($item,"title","")).'\', 0)">';
			}else{
				$item['is_recommended_action'] = '<img src="'.CSS_PATH.'dandelion/images/icons/color/cross.png" class="button" title="Click to set recommended this news" onclick="processChangeRecommended(\''.get_array_value($item,"aid","").'\', \''.removeAllQuote(get_array_value($item,"title","")).'\', 1)">';
			}
			
			$in_home = get_array_value($item,"in_home","0");
			if($in_home == 1){
				$item["in_home_action"] = '<img src="'.CSS_PATH.'dandelion/images/icons/color/accept.png" class="button" title="Click to \'Inctive\' this news" onclick="processChangeValue(\''.get_array_value($item,"aid","").'\', \'admin/news\', \' inactive '.get_array_value($item,"title","").'\', \'in_home=0\')">';
			}else{
				$item["in_home_action"] = '<img src="'.CSS_PATH.'dandelion/images/icons/color/cross.png" class="button" title="Click to \'Active\' this news" onclick="processChangeValue(\''.get_array_value($item,"aid","").'\', \'admin/news\', \' active '.get_array_value($item,"title","").'\', \'in_home=1\')">';
			}
			
			$item["action"] = '';
			if(is_owner_admin_or_higher()){
				$item["action"] .= '<img src="'.CSS_PATH.'dandelion/images/icons/color/bin_closed.png" class="button" title="Click to \'Delete\' this news" onclick="processDelete(\''.get_array_value($item,"aid","").'\', \'admin/news\', \''.get_array_value($item,"title","").'\')">';
			}
	...
	function status($type="")	{
		switch($type)
		{
			...
			case md5('no-command') : 
				...
				$this->data["js_code"] = '';
				break;
			case md5('no-news-upload') : 
				$this->data["message"] = set_message_error('No news found.');
				$this->data["js_code"] = '';
				break;
			default : 
	...
	function detail($aid=""){
		@define("thisAction",'show');
		$this->data["title"] = DEFAULT_TITLE;
		$this->data["view_the_content"] = $this->default_theme_admin . '/news/news_upload_detail';
		$this->data["header_title"] = TXT_TITLE;
