...
class News_upload_front_controller extends Initcontroller {
	...
	function __construct(){
		...
		define("thisFrontTabMenu",'news_upload');
		...
		$this->data["master_news_category"] = $this->news_category->load_news_categories();
	...
	function form(){
		...
		$this->data["view_the_content"] = $this->default_theme_front . '/news/news_upload_form';
	...
	function save(){
		...
		$category_list = "";
		$category = $this->input->get_post('news_category_aid');
		...
		if( !is_blank(get_array_value($_FILES,"file_attach","")) && !is_blank(get_array_value($_FILES["file_attach"],"name","")) ){
			$upload_base_path = "./uploads/".CONST_PROJECT_CODE."/news_upload/".get_datetime_pattern("Y",$publish_date,"").'/'.get_datetime_pattern("m",$publish_date,"").'/'.$cid.'/';
			$upload_base_path_db = "uploads/".CONST_PROJECT_CODE."/news_upload/".get_datetime_pattern("Y",$publish_date,"").'/'.get_datetime_pattern("m",$publish_date,"").'/'.$cid.'/';
			...
			if ( !is_blank(get_array_value($result_file,"error_msg","")) )
			{
				...
				$this->log_status('News Upload : Insert', 'Upload file error : '.$result_file["error_msg"]);
		...
		if($aid > 0){
			$this->log_status('News Upload : Insert', '[title='.$title.'][upload by=('.$posted_by.')('.$posted_email.')'.$posted_ref.'] just added into database by.');
			redirect('news/upload/status/'.md5('success'));
		}else{
			$this->log_error('News Upload : Insert', 'Command insert_record() fail. Can not insert [title='.$title.'][upload by=('.$posted_by.')('.$posted_email.')'.$posted_ref.']');
