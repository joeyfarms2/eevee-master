...
class News_main_back_controller extends News_init_controller {

	function __construct(){
		parent::__construct();

		if(CONST_HAS_NEWS != "1" || (CONST_NEWS_MODE != "1" && !is_root_admin_or_higher())){
			redirect('admin');
		}
		...
		@define("folderName",'news/news_back/main');
		
		define("TXT_TITLE",'Main news management');
		define("TXT_INSERT_TITLE",'Main news management : Add new main news');
		define("TXT_UPDATE_TITLE",'Main news management : Edit main news');
				
		$this->main_model = 'News_main_model';
	}
	...
	function add(){
		...
		$this->session->set_userdata('newsMainBackDataSearchSession','');
	...
	function edit($aid=""){
		...
		if(is_var_array($item_detail)){
			...
			$this->data["message"] = set_message_error('Cannot find the specific news main.');
			$this->data["js_code"] = "$('#name').focus();";
			$this->show();
			return "";
		}
	...
	function save(){
		...
		if($command == "_insert"){
			...
			if($aid > 0){
				$data["aid"] = $aid;
				$this->log_status('Backend : Insert news main', '['.$name.'] just added into database.', $data);
				redirect('admin/news-main/status/'.md5('success'));
			}else{
				$this->log_error('Backend : Insert news main', 'Command insert_record() fail. Can not insert '.$name, $data);
			...
			if($rs){
				...
				if ($aid > 0) $this->log_status('Backend : Update news main',  '['.$name.'] has been updated.', $data);
				if($save_option){
					redirect('admin/news-main/add');
				}else{
					redirect('admin/news-main/status/'.md5('success'));
				}
				return "";
			}else{
				$this->log_error('Backend : Update news main', 'Command update_record() fail. Can not update '.$name.'['.$aid.']', $data);
				$this->data["message"] = set_message_error('Sorry, the system can not save data now. Please try again or contact your administrator.');
				$this->data["js_code"] = '';
				$this->data["command"] = $command;
				$this->data["item_detail"] = $data;
			...
			$this->log_error('Backend : News main', 'Command not found.', $data);
			redirect('admin/news-main/status/'.md5('no-command'));
			return "";
		}
	}
	...
	function ajax_set_value($sid="", $status=""){
		...
		if(!is_var_array($objResult)){
			$msg = set_message_error('Error occurred. Can not find this news main.');
			$result_obj = array("status" => 'error',"msg" => $msg );
			echo json_encode($result_obj);
			return "";
		}
		...
		if ($rs){
			$this->log_status('Backend : News main', 'Set value : "'.$f_name.'" has been set to "'.$f_value.'" for '.$this_obj_info.'.', $objResult);
			$msg = set_message_success('Data has been saved.');
			$result_obj = array("status" => 'success',"msg" => $msg );
			echo json_encode($result_obj);
			return "";
		}else{
			$this->log_error('Backend : News main', 'Set value : Function main->set_value() fail. Can not set "'.$f_name.'" to "'.$f_value.'" for '.$this_obj_info.'.', $objResult);
	...
	function ajax_delete_one(){
		...
		if ($rs){
			$this->log_status('Backend : Delete news main', $this_obj_info.' has been deleted.', $objResult);
			$msg = set_message_success($this_obj_title.' has been deleted.');
			$result_obj = array("status" => 'success',"msg" => $msg );
			echo json_encode($result_obj);
			return "";
		}else{
			$this->log_error('Backend : Delete news main', 'Command delete_records() fail. Can not delete '.$this_obj_info.'.', $objResult);
			$msg = set_message_error('Sorry, the system can not save data now. Please try again or contact your administrator.');
			$result_obj = array("status" => 'error',"msg" => $msg );
			echo json_encode($result_obj);
			return "";
	...
	function ajax_get_main_list($sid){
		...
		$dsSession = $dataSearchSession->userdata('newsMainBackDataSearchSession');		
		// print_r($dsSession);
		...
		$this->session->set_userdata('newsMainBackDataSearchSession',$data_search);	
	...
	function manage_column_detail($result_list){
		...
		foreach($result_list as $item){
			$item["name_action"] = '<a href="'.site_url('admin/news-main/edit/'.get_array_value($item,"aid","")).'">'.get_array_value($item,"name","-").'</a>';
			$status = get_array_value($item,"status","0");
			if($status == 1){
				$item["status_action"] = '<a class="btn btn-success btn-xs" title="Click to \'Inactive\' this." onclick="processChangeValue(\' inactive <strong>'.removeAllQuote(get_array_value($item,"name","")).'</strong>\', \'admin/news-main\', \''.get_array_value($item,"aid","").'\', \'status=0\')"><i class="fa fa-check"></i></a>';
			}else{
				$item["status_action"] = '<a class="btn btn-danger btn-xs" title="Click to \'Active\' this." onclick="processChangeValue(\' active <strong>'.removeAllQuote(get_array_value($item,"name","")).'</strong>\', \'admin/news-main\', \''.get_array_value($item,"aid","").'\', \'status=1\')"><i class="fa fa-ban"></i></a>';
			}

			$item["action"] = '';
			if(is_root_admin_or_higher()){
				$item["action"] .= '<a class="btn btn-danger btn-xs" title="Click to \'Delete\' this." onclick="processDelete(\''.get_array_value($item,"aid","").'\', \'admin/news-main\', \'<strong>'.removeAllQuote(get_array_value($item,"name","")).'</strong>\')"><i class="fa fa-trash-o "></i></a>&nbsp;&nbsp;&nbsp;';
			}
			$result[] = $item;
		}
	...
	function ajax_get_news_main($sid,$news_main_aid){
		$this->load->model($this->main_model,'main');
		$this->main->set_where('aid', $news_main_aid);
