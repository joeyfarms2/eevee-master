	...
	function create_user(){
		...
		if($user_section_aid > 1 ){
			...
			if(is_blank($department_aid)){
				$result_obj = array("status" => 'error',"msg" => 'Parameter missing : Please specify Department id.', "result" => '');
				echo json_encode($result_obj);
				return "";
			}
			...
			$note_1 = $this->input->get_post('department_name');
			if(is_blank($note_1)){
				$result_obj = array("status" => 'error',"msg" => 'Parameter missing : Please specify Department Name.', "result" => '');
				echo json_encode($result_obj);
				return "";
			}
...
*/
		...
		}elseif( CONST_LDAP_AUTHEN_JSON == TRUE && ((is_var_array($user_result) && $user_result['password'] == "") || (!is_var_array($user_result)))) { 
					
					$user_result = $this->chack_json_status($username,$password);
			...
			foreach ($obj as $key => $value) {
				...
				if(is_var_array($value)){
					...
					if($chack == "yes"){
						...
						$this->log_status('Username or password is incorrect. Please try again.');
						$result_obj = array("status" => 'error',"msg" => 'Username or password is incorrect. Please try again.', "result" => '');
						echo json_encode($result_obj);
						return "";
					}
