...
class Webservice_shelf_vdo_controller extends Init_webservice_controller {
	...
	function get_mybookshelf_vdo(){
		...
		if(is_var_array($login_history)){
			...
			$result_list = $this->shelf_vdo->get_shelf_detail($obj);
			...
			foreach($result_list as $item){
				...
				$obj["url"] = get_array_value($item,"url","");
				...
				if(is_var_array($item_vdo)){
					...
					// $result = array();
					//print_r($item);echo "<HR>";
					// $parent_aid = get_array_value($item_vdo,"aid","");
	...
	function get_shelf_vdo_history(){
		$device = trim($this->input->get_post('device'));
		$this->check_device();
		...
		if(is_var_array($login_history)){
			...
			$result_list = $this->shelf_vdo_history->get_shelf_history_detail($obj);
			if(!is_var_array($result_list)){
				$result_obj = array("status" => 'warning',"msg" => 'No record found.', "result" => '');
				echo json_encode($result_obj);
				return "";
	...
	function add_vdo_to_mybookshelf(){
		$device = trim($this->input->get_post('device'));
		...
		if(is_blank($parent_aid)){
			$result_obj = array("status" => 'error',"msg" => 'Parameter missing : Please specify parent_aid.', "result" => '');
			echo json_encode($result_obj);
			return "";
		}
		if(!is_blank($parent_aid)){
			if(!is_number_no_zero($parent_aid)){
				$result_obj = array("status" => 'error',"msg" => 'Incorrect data type : parent_aid must be integer.', "result" => '');
				echo json_encode($result_obj);
				return "";
			}
		}

		// $this->load->model($this->view_all_products_model,'view_all_products');
		// $this->view_all_products->set_where(array("aid"=>$parent_aid, "product_type_aid"=>$product_type_aid, "status"=>"1"));
		// $view_all_products_result = $this->view_all_products->load_record(false);
		// if(!is_var_array($view_all_products_result)){
		...
		//check is ebook?
		//$parent_aid = get_array_value($view_all_products_result,"parent_aid","0");
		...
		$this->load->model($this->shelf_vdo_model,"shelf_vdo");
		$this->shelf_vdo->set_where(array("user_aid"=>$user_aid, "parent_aid"=>$parent_aid, "product_type_aid"=>$product_type_aid));
		$result_list = $this->shelf_vdo->load_record(false);
	...
	function remove_vdo_from_mybookshelf(){
		$device = trim($this->input->get_post('device'));
		$device_id = trim($this->input->get_post('device_id'));
		$this->check_device();
		...
		// $this->view_all_copies->set_where(array("aid"=>$parent_aid, "product_type_aid"=>$product_type_aid, "is_ebook"=>"1", "status"=>"1"));
	...
	function check_vdo_in_mybookshelf(){
		$device = trim($this->input->get_post('device'));
		$device_id = trim($this->input->get_post('device_id'));
		$this->check_device();
