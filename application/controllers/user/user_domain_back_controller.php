...
class User_domain_back_controller extends User_init_controller {
	...
	function __construct(){
		...
		define("thisAdminSubMenu",'user_domain');
		@define("folderName",'user/user_back/domain');
		
		define("TXT_TITLE",'Domain management');
		define("TXT_INSERT_TITLE",'Domain management : Add new domain');
		define("TXT_UPDATE_TITLE",'Domain management : Edit domain');
				
		$this->main_model = 'User_domain_model';
	...
	function show($msg=""){
		...
		$this->data["view_the_content"] = $this->default_theme_admin . '/' . folderName . '/domain_list';
	...
	function form(){
		$this->data["title"] = DEFAULT_TITLE;
		$this->data["view_the_content"] = $this->default_theme_admin . '/' . folderName . '/domain_form';
		$this->load->view($this->default_theme_admin.'/tpl_admin',$this->data);
	}
	...
	function add(){
		...
		$this->session->set_userdata('userDomainBackDataSearchSession','');
	...
	function edit($aid=""){
		...
		if(is_var_array($item_detail)){
			...
			$this->data["message"] = set_message_error('Cannot find the specific domain.');
			$this->data["js_code"] = "$('#name').focus();";
			$this->show();
			return "";
		}
	...
	function save(){
		...
		if($command == "_insert"){
			...
			if($aid > 0){
				$data["aid"] = $aid;
				$this->log_status('Backend : Insert domain', '['.$name.'] just added into database.', $data);
				redirect('admin/user-domain/status/'.md5('success'));
			}else{
				$this->log_error('Backend : Insert domain', 'Command insert_record() fail. Can not insert '.$name, $data);
			...
			if($rs){
				...
				if ($aid > 0) $this->log_status('Backend : Update domain',  '['.$name.'] has been updated.', $data);
				if($save_option){
					redirect('admin/user-domain/add');
				}else{
					redirect('admin/user-domain/status/'.md5('success'));
				}
				return "";
			}else{
				$this->log_error('Backend : Update domain', 'Command update_record() fail. Can not update '.$name.'['.$aid.']', $data);
				$this->data["message"] = set_message_error('Sorry, the system can not save data now. Please try again or contact your administrator.');
				$this->data["js_code"] = '';
				$this->data["command"] = $command;
				$this->data["item_detail"] = $data;
			...
			redirect('admin/user-domain/status/'.md5('no-command'));
			return "";
		}
	}
	...
	function ajax_set_value($sid="", $status=""){
		...
		if(!is_var_array($objResult)){
			$msg = set_message_error('Error occurred. Can not find this domain.');
			$result_obj = array("status" => 'error',"msg" => $msg );
			echo json_encode($result_obj);
			return "";
		}
	...
	function ajax_delete_one(){
		...
		if ($rs){
			$this->log_status('Backend : Delete domain', $this_obj_info.' has been deleted.', $objResult);
			$msg = set_message_success($this_obj_title.' has been deleted.');
			$result_obj = array("status" => 'success',"msg" => $msg );
			echo json_encode($result_obj);
			return "";
		}else{
			$this->log_error('Backend : Delete domain', 'Command delete_records() fail. Can not delete '.$this_obj_info.'.', $objResult);
			$msg = set_message_error('Sorry, the system can not save data now. Please try again or contact your administrator.');
			$result_obj = array("status" => 'error',"msg" => $msg );
			echo json_encode($result_obj);
			return "";
	...
	function ajax_get_main_list($sid){
		...
		$dsSession = $dataSearchSession->userdata('userDomainBackDataSearchSession');		
		// print_r($dsSession);
		...
		$header_list = array();
		$header_list[] = array("sort_able"=>'1',"title_show"=>'Domain',"field_order"=>'name',"col_show"=>'name_action',"title_class"=>'w100 hcenter',"result_class"=>'hleft');
		$header_list[] = array("sort_able"=>'1',"title_show"=>'Weight',"field_order"=>'weight',"col_show"=>'weight',"title_class"=>'w100 hcenter',"result_class"=>'hleft');
		...
		$this->session->set_userdata('userDomainBackDataSearchSession',$data_search);	
	...
	function manage_column_detail($result_list){
		...
		foreach($result_list as $item){
			$item["name_action"] = '<a href="'.site_url('admin/user-domain/edit/'.get_array_value($item,"aid","")).'">'.get_array_value($item,"name","-").'</a>';
			...
			if($status == 1){
				$item["status_action"] = '<a class="btn btn-success btn-xs" title="Click to \'Inactive\' this." onclick="processChangeValue(\' inactive <strong>'.removeAllQuote(get_array_value($item,"name","")).'</strong>\', \'admin/user-domain\', \''.get_array_value($item,"aid","").'\', \'status=0\')"><i class="fa fa-check"></i></a>';
			}else{
				$item["status_action"] = '<a class="btn btn-danger btn-xs" title="Click to \'Active\' this." onclick="processChangeValue(\' active <strong>'.removeAllQuote(get_array_value($item,"name","")).'</strong>\', \'admin/user-domain\', \''.get_array_value($item,"aid","").'\', \'status=1\')"><i class="fa fa-ban"></i></a>';
			}

			$item["action"] = '';			
			if(is_owner_admin_or_higher()){
				$item["action"] .= '<a class="btn btn-danger btn-xs" title="Click to \'Delete\' this." onclick="processDelete(\''.get_array_value($item,"aid","").'\', \'admin/user-domain\', \'<strong>'.removeAllQuote(get_array_value($item,"name","")).'</strong>\')"><i class="fa fa-trash-o "></i></a>&nbsp;&nbsp;&nbsp;';
			}
