...
class Report_user_download_controller extends Project_init_controller {
	...
	function __construct()
	{
		...
		@define("folderName","report/user_download/");
	...
	function index()
	{
		$this->user_download_log();
	}
	
	function user_download_log(){
		@define("thisAction","user_download_log");
		@define('thisAdminSubMenu','report_user_download_log');
		$this->data["title"] = DEFAULT_TITLE;
		$this->data["view_the_content"] = $this->default_theme_admin . '/report/user_download/user_download_log_page';
		$this->data["message"] = "";
	...
	function ajax_get_user_download_log_list($sid=""){
		...
		if(is_blank($search_order_by)){
			$order_by = "*first_name_th";
			$order_by_option = "asc";
			$order_by_txt = "*first_name_th asc, email asc";
			$order_by_option_txt = "";
		}else{
			list($order_by, $order_by_option) = preg_split("/ /", $search_order_by, 2);
			if($order_by == "*first_name_th"){
				$order_by_txt = "*first_name_th ".$order_by_option.", email ".$order_by_option."";
				$order_by_option_txt = "";
			}else{
				$order_by_txt = $order_by;
				$order_by_option_txt = $order_by_option;
			}
		}
		$data_search["search_order_by"] = $search_order_by;
		$sorting = array("order_by"=>$order_by , "order_by_option"=>$order_by_option);

		$this->user_download_report->set_order_by($order_by_txt." ".$order_by_option_txt);
		$this->user_download_report->set_limit($start_record,$search_record_per_page);
		$result_list = $this->user_download_report->load_records(true);
		$result_list = $this->manage_column_user_download_log($result_list);
		$this->db->flush_cache();
	...
	function manage_column_user_download_log($result_list){
		$result = "";
		if(!is_var_array($result_list)){
			return "";
		}
	...
	function export_user_download_log(){
		if(!is_owner_admin_or_higher()){
