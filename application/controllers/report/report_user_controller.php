...
class Report_user_controller extends Project_init_controller {
	...
	function user_log(){
		...
		@define('thisAdminSubMenu','report_user_log');
		$this->data["title"] = DEFAULT_TITLE;
		$this->data["view_the_content"] = $this->default_theme_admin . '/report/user/user_log_page';
		$this->data["message"] = "";
	...
	function ajax_get_user_log_list($sid=""){
		...
		// $this->user_history->set_where(array("action"=>"login"));
		...
		$this->user_history->set_or_like($data_where);
		...
		$this->user_history->set_where_in($data_where);
		$this->user_history->set_where("user.user_role_aid >= '".getSessionUserRoleAid()."'");
		...
		if(!is_blank($created_date_from)){
			$this->user_history->set_where($this->user_history->get_table_name().'.created_date >=', get_datetime_pattern("db_date_format",$created_date_from,"")." 00:00:00");
		}
		if(!is_blank($created_date_to)){
			$this->user_history->set_where($this->user_history->get_table_name().'.created_date <=', get_datetime_pattern("db_date_format",$created_date_to,"")." 23:59:59");
		}		
		...
		$total_record = $this->user_history->count_records(true);
		if($total_record > 0){
		...
		$this->user_history->set_order_by($order_by." ".$order_by_option);
		$this->user_history->set_limit($start_record,$search_record_per_page);
		$result_list = $this->user_history->load_records(true);
		$result_list = $this->manage_column_user_log($result_list);
		$this->db->flush_cache();
		...
		$header_list[] = array("sort_able"=>"1","title_show"=>"Login By","field_order"=>"created_by","col_show"=>"user_info_show","title_class"=>"w200 hcenter","result_class"=>"hleft");
		$header_list[] = array("sort_able"=>"1","title_show"=>"Channel","field_order"=>"device","col_show"=>"device_name","title_class"=>"w50 hcenter","result_class"=>"hleft");
		$header_list[] = array("sort_able"=>"1","title_show"=>"IP","field_order"=>"ip","col_show"=>"ip","title_class"=>"w50 hcenter","result_class"=>"hleft");
		$header_list[] = array("sort_able"=>"1","title_show"=>"Login Date","field_order"=>"created_date","col_show"=>"created_date_txt","title_class"=>"hcenter","result_class"=>"hleft");
	...
	function manage_column_user_log($result_list){
		$result = "";
		if(!is_var_array($result_list)){
			return "";
		}
	...
	function export_user_log($sid=""){
		...
		if(is_var_array($result_list)){
			...
			$objPHPExcel->getProperties()->setTitle("User login List");
			$objPHPExcel->getProperties()->setDescription("User login list");
			// Set Default Style
			$objPHPExcel->setActiveSheetIndex(0);
			$objPHPExcel->getActiveSheet()->getDefaultStyle()->getFont()->setName('Arial');
			$objPHPExcel->getActiveSheet()->getDefaultStyle()->getFont()->setSize(10); 
			// Rename Sheet
			$objPHPExcel->getActiveSheet()->setTitle('User login List');
			// Set column width
			...
			$objPHPExcel->getActiveSheet()->setCellValue('A1', "Login By");
			$objPHPExcel->getActiveSheet()->setCellValue('B1', "Channel");
			$objPHPExcel->getActiveSheet()->setCellValue('C1', "IP");
			$objPHPExcel->getActiveSheet()->setCellValue('D1', "Login Date");
			...
			foreach($result_list as $item){
				...
				$objPHPExcel->getActiveSheet()->setCellValue('B'.$irow, get_array_value($item,"device_name",""));
				$objPHPExcel->getActiveSheet()->setCellValue('C'.$irow, get_array_value($item,"ip",""));
				$objPHPExcel->getActiveSheet()->setCellValue('D'.$irow, get_array_value($item,"created_date_txt",""));
				$irow++;
			}
