...
class Report_shelf_download_controller extends Project_init_controller {
	...
	function __construct()
	{
		...
		@define("folderName","report/shelf_download/");
	...
	function index()
	{
		$this->shelf_download_log();
	}
	
	function shelf_download_log(){
		@define("thisAction","shelf_download_log");
		@define('thisAdminSubMenu','report_shelf_download_log');
		$this->data["title"] = DEFAULT_TITLE;
		$this->data["view_the_content"] = $this->default_theme_admin . '/report/shelf_download/shelf_download_log_page';
		$this->data["message"] = "";
	...
	function ajax_get_shelf_download_log_list($sid=""){
		...
		$this->load->model($this->view_all_download_history,"shelf_download_report");
		$this->db->start_cache();		
		$this->db->select('view_all_download_history.*, count(*) as total_download');
		$this->db->group_by("copy_aid, product_type_aid");

		// $this->shelf_download_report->set_where(array("action"=>"login"));
		...
		$this->shelf_download_report->set_or_like($data_where);
		...
		$this->shelf_download_report->set_where_in($data_where);
		...
		if(!is_blank($created_date_from)){
			$this->shelf_download_report->set_where($this->shelf_download_report->get_table_name().'.updated_date >=', get_datetime_pattern("db_date_format",$created_date_from,"")." 00:00:00");
		}
		if(!is_blank($created_date_to)){
			$this->shelf_download_report->set_where($this->shelf_download_report->get_table_name().'.updated_date <=', get_datetime_pattern("db_date_format",$created_date_to,"")." 23:59:59");
		}		
		...
		$total_record = $this->shelf_download_report->load_count_records(false);
		if($total_record > 0){
		...
		$this->shelf_download_report->set_order_by($order_by." ".$order_by_option);
		$this->shelf_download_report->set_limit($start_record,$search_record_per_page);
		$result_list = $this->shelf_download_report->load_records(true);
		$result_list = $this->manage_column_shelf_download_log($result_list);
		$this->db->flush_cache();
	...
	function manage_column_shelf_download_log($result_list){
		$result = "";
		if(!is_var_array($result_list)){
			return "";
		}
	...
	function export_download_log(){
		...
		// echo "order_by = $order_by , order_by_option = $order_by_option";
		$this->shelf_download_report->set_order_by($order_by." ".$order_by_option);
		...
		if(is_var_array($result_list)){
			...
			$objPHPExcel->getProperties()->setTitle("Download List");
			$objPHPExcel->getProperties()->setDescription("Download list");
			// Set Default Style
			$objPHPExcel->setActiveSheetIndex(0);
			$objPHPExcel->getActiveSheet()->getDefaultStyle()->getFont()->setName('Arial');
			$objPHPExcel->getActiveSheet()->getDefaultStyle()->getFont()->setSize(10); 
			// Rename Sheet
			$objPHPExcel->getActiveSheet()->setTitle('Download List');
			// Set column width
			...
			$objPHPExcel->getActiveSheet()->setCellValue('C1', "#Download");
