...
class Event_category_back_controller extends Event_init_controller {
	...
	function __construct(){
		...
		@define("folderName",'event/event_back/category');
		...
		$this->main_model = 'Event_category_model';		
	}
	...
	function add(){
		...
		$this->session->set_userdata('eventCategoryBackDataSearchSession','');
	...
	function save(){
		...
		if($command == "_insert"){
			...
			if($aid > 0){
				$data["aid"] = $aid;
				$this->log_status('Backend : Insert event category', '['.$name.'] just added into database.', $data);
				redirect('admin/event-category/status/'.md5('success'));
			}else{
				$this->log_error('Backend : Insert event category', 'Command insert_record() fail. Can not insert '.$name, $data);
			...
			if($rs){
				...
				if ($aid > 0) $this->log_status('Backend : Update event category',  '['.$name.'] has been updated.', $data);
				if($save_option){
					redirect('admin/event-category/add');
				}else{
					redirect('admin/event-category/status/'.md5('success'));
				}
				return "";
			}else{
				$this->log_error('Backend : Update event category', 'Command update_record() fail. Can not update '.$name.'['.$aid.']', $data);
				$this->data["message"] = set_message_error('Sorry, the system can not save data now. Please try again or contact your administrator.');
				$this->data["js_code"] = '';
				$this->data["command"] = $command;
				$this->data["item_detail"] = $data;
			...
			$this->log_error('Backend : Event category', 'Command not found.', $data);
			redirect('admin/event-category/status/'.md5('no-command'));
			return "";
		}
	}
	...
	function check_duplicate($data="",$command=""){
		...
		$event_main_aid = get_array_value($data,"event_main_aid","");
		$user_owner_aid = get_array_value($data,"user_owner_aid","");

		$this->main->set_where(array("user_owner_aid"=>$user_owner_aid, "event_main_aid"=>$event_main_aid));
	...
	function ajax_get_main_list($sid){
		...
		$dsSession = $dataSearchSession->userdata('eventCategoryBackDataSearchSession');		
		// print_r($dsSession);
		...
		$search_event_main = $this->getDataFromInput('search_event_main');
		$data_where = "";
		...
		if (is_root_admin_or_higher()) {
			$header_list[] = array("sort_able"=>'1',"title_show"=>'Event Main',"field_order"=>'event_main_aid',"col_show"=>'event_main_name',"title_class"=>'w150 hcenter',"result_class"=>'hleft');
		}
		...
		$this->session->set_userdata('eventCategoryBackDataSearchSession',$data_search);	
	...
	function manage_column_detail($result_list){
		...
		foreach($result_list as $item){
			$item["name_action"] = '<a href="'.site_url('admin/event-category/edit/'.get_array_value($item,"aid","")).'">'.get_array_value($item,"name","-").'</a>';
			$status = get_array_value($item,"status","0");
			if($status == 1){
				$item["status_action"] = '<a class="btn btn-success btn-xs" title="Click to \'Inactive\' this book." onclick="processChangeValue(\' inactive <strong>'.removeAllQuote(get_array_value($item,"name","")).'</strong>\', \'admin/event-category\', \''.get_array_value($item,"aid","").'\', \'status=0\')"><i class="fa fa-check"></i></a>';
			}else{
				$item["status_action"] = '<a class="btn btn-danger btn-xs" title="Click to \'Active\' this book." onclick="processChangeValue(\' active <strong>'.removeAllQuote(get_array_value($item,"name","")).'</strong>\', \'admin/event-category\', \''.get_array_value($item,"aid","").'\', \'status=1\')"><i class="fa fa-ban"></i></a>';
			}

			$item["action"] = '';			
			if(is_owner_admin_or_higher()){
				$item["action"] .= '<a class="btn btn-danger btn-xs" title="Click to \'Delete\' this book." onclick="processDelete(\''.get_array_value($item,"aid","").'\', \'admin/event-category\', \'<strong>'.removeAllQuote(get_array_value($item,"name","")).'</strong>\')"><i class="fa fa-trash-o "></i></a>&nbsp;&nbsp;&nbsp;';
			}
	...
	function ajax_get_category_by_event_main($sid,$event_main_aid){
		$this->load->model($this->main_model,'main');
		$this->main->set_where('event_main_aid', $event_main_aid);
