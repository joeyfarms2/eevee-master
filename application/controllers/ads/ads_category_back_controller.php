...
class Ads_category_back_controller extends Project_init_controller {
	...
	function __construct(){
		...
		@define("folderName",'ads/ads_back/category');
		...
		$this->main_model = 'Ads_category_model';		
	}
	...
	function add(){
		...
		$this->session->set_userdata('adsCategoryBackDataSearchSession','');
	...
	function save(){
		...
		if($command == "_insert"){
			...
			if($aid > 0){
				$data["aid"] = $aid;
				$this->log_status('Backend : Insert ads category', '['.$name.'] just added into database.', $data);
				redirect('admin/ads-category/status/'.md5('success'));
			}else{
				$this->log_error('Backend : Insert ads category', 'Command insert_record() fail. Can not insert '.$name, $data);
			...
			if($rs){
				...
				if ($aid > 0) $this->log_status('Backend : Update ads category',  '['.$name.'] has been updated.', $data);
				if($save_option){
					redirect('admin/ads-category/add');
				}else{
					redirect('admin/ads-category/status/'.md5('success'));
				}
				return "";
			}else{
				$this->log_error('Backend : Update ads category', 'Command update_record() fail. Can not update '.$name.'['.$aid.']', $data);
				$this->data["message"] = set_message_error('Sorry, the system can not save data now. Please try again or contact your administrator.');
				$this->data["js_code"] = '';
				$this->data["command"] = $command;
				$this->data["item_detail"] = $data;
			...
			$this->log_error('Backend : Ads category', 'Command not found.', $data);
			redirect('admin/ads-category/status/'.md5('no-command'));
			return "";
		}
	}
	...
	function check_duplicate($data="",$command=""){
		...
		$this->main->set_and_or_where(array("name"=>$name, "cid"=>$cid));
		if(!is_blank($aid)){
	...
	function ajax_get_main_list($sid){
		...
		$dsSession = $dataSearchSession->userdata('adsCategoryBackDataSearchSession');		
		// print_r($dsSession);
		...
		$header_list = array();
		$header_list[] = array("sort_able"=>'1',"title_show"=>'Category cid',"field_order"=>'cid',"col_show"=>'cid',"title_class"=>'w250 hcenter',"result_class"=>'hleft');
		...
		$this->session->set_userdata('adsCategoryBackDataSearchSession',$data_search);	
	...
	function manage_column_detail($result_list){
		...
		foreach($result_list as $item){
			$item["name_action"] = '<a href="'.site_url('admin/ads-category/edit/'.get_array_value($item,"aid","")).'">'.get_array_value($item,"name","-").'</a>';
			$status = get_array_value($item,"status","0");
			if($status == 1){
				$item["status_action"] = '<a class="btn btn-success btn-xs" title="Click to \'Inactive\' this book." onclick="processChangeValue(\' inactive <strong>'.removeAllQuote(get_array_value($item,"name","")).'</strong>\', \'admin/ads-category\', \''.get_array_value($item,"aid","").'\', \'status=0\')"><i class="fa fa-check"></i></a>';
			}else{
				$item["status_action"] = '<a class="btn btn-danger btn-xs" title="Click to \'Active\' this book." onclick="processChangeValue(\' active <strong>'.removeAllQuote(get_array_value($item,"name","")).'</strong>\', \'admin/ads-category\', \''.get_array_value($item,"aid","").'\', \'status=1\')"><i class="fa fa-ban"></i></a>';
			}

			$item["action"] = '';			
			if(is_root_admin_or_higher()){
				$item["action"] .= '<a class="btn btn-danger btn-xs" title="Click to \'Delete\' this book." onclick="processDelete(\''.get_array_value($item,"aid","").'\', \'admin/ads-category\', \'<strong>'.removeAllQuote(get_array_value($item,"name","")).'</strong>\')"><i class="fa fa-trash-o "></i></a>&nbsp;&nbsp;&nbsp;';
			}
