...
<section id="projects">
	<div class="container mt15 pan">
		...
		<?php //if(is_var_array($news_result)) { ?>
			<div class="row mbl">
				<div class="col-sm-6 col-xs-12"><h3 id="page_category_name" class="textSub mts mbn"></h3></div>
				...
				<!-- <div class="col-sm-2 col-xs-4 pls">
					<a id="btn_post_new_news" class="btn btn-primary" href="javascript:void(0);">
						<i class="fa fa-plus mrs"></i> Post news</a>
					</a>
				</div> -->
			</div>
		<?php //} ?>
	...
	$(document).ready(function($){
		...
		*/
		...
		$('#ddl_news_category').change(function() {
     	...
     	$('#btn_post_new_news').click(function() {
     		...
     		$('#modal_front_post_news').find('.modal-title').html('Post news');
     		$('#modal_front_post_news').modal('show');
     		$('#command').val('_insert');

     		$('#news_aid').val('');
       		$('#title').val('');
       		tinyMCE.get('description_1').setContent('');
       		$('#ref_link').val('');
	...
	$.ajax({
       ...
       data: { news_aid: $(e).attr('data-aid') },
		beforeSend:function() {
			...
			$('#modal_front_post_news').find('.result-msg').html('<div><i class="fa fa-spinner fa-spin"></i> Loading...</div>').removeClass('hidden');
