...
<input Type="Hidden" Name="itm" value="<?=get_array_value($order_result,"package_name","")?>"/>
...
<input Type="Hidden" Name="postURL" value="<?=site_url('paysbuy/save-point-back-from-paysbuy-front')?>"/>
<input Type="Hidden" Name="reqURL" value="<?=site_url('paysbuy/save-point-back-from-paysbuy-back')?>"/>
...
<div id="result-msg-box" class=" mt30 mb30"><img src="<?=IMAGE_PATH?>icons/loader.gif" />&nbsp;&nbsp;กรุณารอสักครู่ ระบบกำลังนำท่านไปยัง&nbsp;&nbsp;<img src="<?=IMAGE_PATH?>payment/paypal-m.jpg" /></div></div>
</form>

<?php if(CONST_MODE == "2"){ ?>
<div class="develope-mode">This block just for develop mode only!! must be comment this form on UAT.
	<BR><BR>
	<form name="frm_back" id="frm_back" method="post" action="<?=site_url('paysbuy/save-point-back-from-paysbuy-front')?>">
	...
	<input type="submit" class="btn btn-sm btn-danger" value="Go!!"/>
	</form>
</div>
<?php } ?>
<script type="text/javascript">
function test_save_success(){
	
}
function test_save_reject(){
