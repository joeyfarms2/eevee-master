...
if($init_adv_search != "clear"){
	...
	$data_search = $dataSearchSession->userdata('newsBackDataSearchSession'); 
}
...
$search_news_category = get_array_value($data_search,"search_news_category","");
$master_news_category = @$master_news_category;
...
<input type="hidden" id="search_order_by" name="search_order_by" value="<?=$search_order_by?>" />
	<div class="row">
		<div class="col-xs-12 m-bot15">
			<a href="<?=site_url('admin/news/add')?>" class="btn btn-primary">
			...
			<section class="panel">
				...
				<div id="adv-area" class="panel-body" style="display: none;">
					...
					<div class="form-group">
						...
						<div class="col-md-12 col-lg-8">
							<label class="checkbox-inline">
								<input type="checkbox" id="search_news_category_all" name="search_news_category_all" onclick="changeCheckAll('search_news_category_all','search_news_category[]',false,false);showSearchResult();" />All
							</label>
							<?php 
								if(is_var_array($master_news_category)){ 
									foreach($master_news_category as $m_item){
							?>
									<label class="checkbox-inline">
										<input type="checkbox" id="search_news_category_<?=get_array_value($m_item,"aid","")?>" name="search_news_category[]" onclick="changeCheckItem('search_news_category_all','search_news_category[]',false,true);showSearchResult();" value="<?=get_array_value($m_item,"aid","")?>" <?php if(is_in_array(get_array_value($m_item,"aid",""),$search_news_category)) echo 'checked="checked"';?>  /><?=get_array_value($m_item,"name","")?>
									</label>
								<?php } //foreach ?>
							<?php } //if ?>
						</div>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		changeCheckItem('search_news_category_all','search_news_category[]',false,true);
