...
if($init_adv_search != "clear"){
	...
	$data_search = $dataSearchSession->userdata('productMainFieldBackDataSearchSession'); 
}
...
$product_main_aid = get_array_value($product_main_result,"aid","0");

// print_r($data_search);
?>
<form id="frm_product_main_field" name="frm_product_main_field" method="POST" action="" class="form-horizontal tasi-form">
...
<input type="hidden" id="product_main_aid" name="product_main_aid" value="<?=$product_main_aid?>" />
	...
	<div class="row">
		<div class="col-xs-12 m-bot15">
			<a href="<?=site_url('admin/product-main/show')?>" class="btn btn-danger">
				<i class="fa fa-reply"></i> Back
			</a>&nbsp;
			<a href="<?=site_url('admin/product-main-field/'.$product_main_aid.'/add')?>" class="btn btn-primary">
				<i class="fa fa-plus"></i> Add new
			</a>
			<?php if(is_root_admin_or_higher()){ ?>
			<a href="<?=site_url('admin/product-main-field/'.$product_main_aid.'/get-init-field')?>" class="btn btn-primary">
				<i class="fa fa-plus"></i> Get init
			</a>
			<?php } ?>
		</div>
	</div>
				...
				<div id="adv-area" class="panel-body" style="display: none;">
					...
					<div class="form-group">
						<div class="col-lg-offset-2 col-lg-8">
							...
							<a class="btn btn-default" onclick="ajaxClearSearchResult('productMainFieldBackDataSearchSession')" />Clear</a>
						</div>
					</div>
					<!-- End : Button -->
				</div>
