...
if($init_adv_search != "clear"){
	...
	$data_search = $dataSearchSession->userdata('productMainBackDataSearchSession'); 
}
...
<input type="hidden" id="search_order_by" name="search_order_by" value="<?=$search_order_by?>" />
	...
	<div class="row">
		<div class="col-xs-12 m-bot15">
			<a href="<?=site_url('admin/product-main/add')?>" class="btn btn-primary">
		...
		<div class="col-xs-12">
			<section class="panel">
				...
				<div id="adv-area" class="panel-body" style="display: none;">
					...
					<div class="form-group">
						<label class="col-md-12 col-lg-2 control-label">Product type</label>
						<div class="col-md-12 col-lg-8">
							<label class="checkbox-inline">
								<input type="checkbox" id="search_product_type_all" name="search_product_type_all" onclick="changeCheckAll('search_product_type_all','search_product_type[]',false,false);showSearchResult();" />All
							</label>
							<?php 
							...
							?>
									<label class="checkbox-inline">
										<input type="checkbox" id="search_product_type_<?=get_array_value($m_item,"aid","")?>" name="search_product_type[]" onclick="changeCheckItem('search_product_type_all','search_product_type[]',false,true);showSearchResult();" value="<?=get_array_value($m_item,"aid","")?>" <?php if(is_in_array(get_array_value($m_item,"aid",""),$search_product_type)) echo 'checked="checked"';?>  /><?=get_array_value($m_item,"name","")?>
									</label>
								<?php } //foreach ?>
							<?php } //if ?>
						</div>
					...
					<div class="form-group">
						<div class="col-lg-offset-2 col-lg-8">
							...
							<a class="btn btn-default" onclick="ajaxClearSearchResult('productMainBackDataSearchSession')" />Clear</a>
						</div>
					</div>
					<!-- End : Button -->
				</div>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		changeCheckItem('search_product_type_all','search_product_type[]',false,true);
