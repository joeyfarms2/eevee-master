...
if($init_adv_search != "clear"){
	...
	$data_search = $dataSearchSession->userdata('productCategoryBackDataSearchSession'); 
}
			...
			<section class="panel">
				...
				<div id="adv-area" class="panel-body" style="display: none;">
					...
					<div class="form-group">
						...
						<div class="col-md-12 col-lg-8">
							<label class="checkbox-inline">
								<input type="checkbox" id="search_product_main_all" name="search_product_main_all" onclick="changeCheckAll('search_product_main_all','search_product_main[]',false,false);showSearchResult();" />All
							</label>
							<?php 
							...
							?>
									<label class="checkbox-inline">
										<input type="checkbox" id="search_product_main_<?=get_array_value($m_item,"aid","")?>" name="search_product_main[]" onclick="changeCheckItem('search_product_main_all','search_product_main[]',false,true);showSearchResult();" value="<?=get_array_value($m_item,"aid","")?>" <?php if(is_in_array(get_array_value($m_item,"aid",""),$search_product_main)) echo 'checked="checked"';?>  /><?=get_array_value($m_item,"name","")?>
									</label>
								<?php } //foreach ?>
							<?php } //if ?>
						</div>
					...
					<div class="form-group">
						<div class="col-lg-offset-2 col-lg-8">
							...
							<a class="btn btn-default" onclick="ajaxClearSearchResult('productCategoryBackDataSearchSession')" />Clear</a>
						</div>
					</div>
					<!-- End : Button -->
				</div>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		changeCheckItem('search_product_main_all','search_product_main[]',false,true);
