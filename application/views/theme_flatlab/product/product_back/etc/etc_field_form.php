...
<form id="frm_product" name="frm_product" method="POST" action="<?=site_url('admin/product/etc-field/save')?>" class="cmxform form-horizontal tasi-form">
	...
	<div class="row">
		<div class="col-md-12">
			<section class="panel">
				...
				<?php 
					$product_type_field_aid = get_array_value($field_item_detail,"product_type_field_aid","");
					$readonly = "";
					if($product_type_field_aid != ""){
						$readonly = "readonly";
					}
				?>

					<div class="form-group">
						<label class="col-md-12 col-lg-2 control-label" for="name">Field Name : <?=$product_type_field_aid?></label>
						<div class="col-md-12 col-lg-8 input-group">
							<input class="form-control" type="text" id="name" name="name" value="<?=get_array_value($field_item_detail,"product_type_field_name","")?>" maxlength="255" <?=$readonly?> />
						</div>
					</div>
			...
			<section class="panel">
				...
				<div class="panel-body">
					...
					<a class="btn btn-default" onclick="processRedirect('admin/product-<?=$product_main_url?>/etc/edit/<?=$parent_aid?>/field');" />Cancel</a>
				</div>
				<!-- End : Button -->
			</section>
		</div>
